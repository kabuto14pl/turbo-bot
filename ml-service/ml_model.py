...
class TradingMLEnsemble:
    ...
    def __init__(self, use_gpu=False):
        ...
        self.xgb_clf = xgb.XGBClassifier(
            n_estimators=500,      # P#retrain: 300→500 more trees
            max_depth=4,           # P#retrain: 5→4 less overfitting
            learning_rate=0.01,    # P#retrain: 0.03→0.01 slower convergence
            subsample=0.70,        # P#retrain: 0.75→0.70
            colsample_bytree=0.6,  # P#retrain: 0.7→0.6 more feature dropout
            min_child_weight=15,   # P#retrain: 10→15 require more samples per leaf
            reg_alpha=0.5,         # P#retrain: 0.3→0.5 more L1 regularization
            reg_lambda=3.0,        # P#retrain: 2.0→3.0 more L2 regularization
            gamma=0.2,             # P#retrain: 0.1→0.2 stricter split threshold
        ...
        self.lgb_clf = lgb.LGBMClassifier(
            n_estimators=500,      # P#retrain: 300→500
            max_depth=4,           # P#retrain: 5→4
            learning_rate=0.01,    # P#retrain: 0.03→0.01
            subsample=0.70,        # P#retrain: 0.75→0.70
            colsample_bytree=0.6,  # P#retrain: 0.7→0.6
            min_child_weight=15,   # P#retrain: 10→15
            reg_alpha=0.5,         # P#retrain: 0.3→0.5
            reg_lambda=3.0,        # P#retrain: 2.0→3.0
            min_split_gain=0.2,    # P#retrain: 0.1→0.2
            num_leaves=16,         # P#retrain: constrain complexity (2^4)
            device=device,
            verbose=-1,
            random_state=42,
        )
        ...
        self.xgb_reg = xgb.XGBRegressor(
            n_estimators=300,      # P#retrain: 200→300
            max_depth=3,           # P#retrain: 4→3
            learning_rate=0.01,    # P#retrain: 0.03→0.01
            subsample=0.70,
