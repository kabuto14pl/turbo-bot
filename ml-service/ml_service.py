...
async def predict(req: PredictRequest):
    ...
    return PredictResponse(
        ...
        model_cv_accuracy=model.training_metrics.get('cv_mean_accuracy', 0) if model.trained else 0,
...
# ============================================================================
# P#152: Live Funding Rate Monitor (Kraken Futures)
# ============================================================================
from backtest_pipeline.kraken_funding_provider import KrakenFundingDataProvider, SYMBOL_MAP

# Cache providers per symbol (reuse across requests)
_funding_providers: Dict[str, KrakenFundingDataProvider] = {}

def _get_funding_provider(symbol: str) -> Optional[KrakenFundingDataProvider]:
    """Get or create a funding rate provider for a symbol."""
    if symbol not in _funding_providers:
        provider = KrakenFundingDataProvider(symbol)
        if provider.is_supported():
            provider.fetch_and_cache()
            _funding_providers[symbol] = provider
        else:
            return None
    return _funding_providers.get(symbol)

@app.get("/funding-rate")
async def funding_rate(symbol: str = "BTCUSDT"):
    """
    Get current Kraken Futures funding rate for a symbol.
    
    Returns the latest 8h aggregated funding rate + stats.
    Auto-refreshes cache every 4 hours.
    """
    provider = _get_funding_provider(symbol.upper())
    if provider is None:
        return {
            'symbol': symbol,
            'supported': False,
            'error': f'{symbol} not available on Kraken Futures',
            'available_symbols': [k for k, v in SYMBOL_MAP.items() if v is not None],
        }
    ...
    # Get latest rate
    latest_rate = None
    if provider.rates_df is not None and len(provider.rates_df) > 0:
        latest_row = provider.rates_df.iloc[-1]
        latest_rate = float(latest_row['funding_rate'])
    
    return {
        'symbol': symbol,
        'supported': True,
        'current_rate_8h': latest_rate,
        'signal': 1.0 if (latest_rate or 0) > 0.00005 else (-1.0 if (latest_rate or 0) < -0.00005 else 0.0),
        'stats': stats,
    }

@app.get("/funding-rate/all")
async def funding_rate_all():
    """Get funding rates for all supported symbols."""
    results = {}
    for symbol in ['BTCUSDT', 'ETHUSDT', 'SOLUSDT', 'XRPUSDT']:
        provider = _get_funding_provider(symbol)
        if provider and provider.rates_df is not None and len(provider.rates_df) > 0:
            latest = float(provider.rates_df.iloc[-1]['funding_rate'])
            stats = provider.get_stats()
            results[symbol] = {
                'current_rate_8h': latest,
                'signal': 1.0 if latest > 0.00005 else (-1.0 if latest < -0.00005 else 0.0),
                'mean_rate_8h': stats.get('mean_rate_8h', 0),
                'positive_pct': stats.get('positive_pct', 0),
                'estimated_annual_yield': stats.get('estimated_annual_yield', 0),
            }
        else:
            results[symbol] = {'current_rate_8h': None, 'signal': 0.0}
    return results
