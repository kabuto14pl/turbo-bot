...
## 🚨� KRYTYCZNA ZASADA NR 1 - CZYTAJ ZA KAŻDYM RAZEM! 🔴🚨

**⚠️ BEZWZGLĘDNY WYMÓG PRZED KAŻDĄ AKCJĄ:**

### **ZAWSZE, PRZENIGDY NIE ZAPOMNIJ:**

1. **PRZECZYTAJ CAŁE INSTRUKCJE** (.github/copilot-instructions.md) **PRZED JAKĄKOLWIEK AKCJĄ**
2. **ZAWSZE PYTAJ UŻYTKOWNIKA** przed utworzeniem jakiegokolwiek nowego pliku

### **📋 OBOWIĄZKOWE PYTANIE PRZED TWORZENIEM PLIKU:**

```
❓ Czy utworzyć nowy plik: [nazwa_pliku.ts]?
   Cel: [opis co będzie w pliku]
   Rozmiar: ~[szacowana liczba linii] LOC

   Wpisz 'tak' aby kontynuować lub 'nie' aby anulować.
```

### **✅ PROCES TWORZENIA NOWEGO PLIKU:**

1. ✅ ZAPYTAJ użytkownika (użyj powyższego formatu)
2. ✅ POCZEKAJ na odpowiedź
3. ✅ TYLKO po "tak" - utwórz plik
4. ✅ NIGDY nie twórz bez zgody!

**🔴 JEŚLI NIE ZAPYTASZ PRZED UTWORZENIEM PLIKU - POPEŁNISZ BŁĄD! 🔴**
...
## 📊 DASHBOARD - ABSOLUTNY ZAKAZ TWORZENIA NOWYCH! 📊

### **🚫 KRYTYCZNA ZASADA: JEDEN DASHBOARD - ZAKAZ DUPLIKACJI!**

**JEDYNY DOZWOLONY DASHBOARD**: `/dashboard` (React + TypeScript + Vite)

### **🚫 ABSOLUTNIE ZABRONIONE:**

- ❌ Tworzenie JAKICHKOLWIEK nowych dashboardów (HTML, JS, TS, React)
- ❌ Duplikowanie funkcji dashboardu w innych plikach
- ❌ Tworzenie "alternatywnych" czy "uproszczonych" wersji dashboardu
- ❌ Budowanie dashboardów w innych katalogach (root, /src/, /trading-bot/)
- ❌ Tworzenie nowych serwerów dashboard (dashboard-server.js, etc.)
- ❌ Migracja dashboardu do innych technologii bez wyraźnego polecenia

### **✅ WYMAGANE ZAWSZE:**

- ✅ **WSZYSTKIE zmiany dashboardu** TYLKO w `/dashboard`
- ✅ Używanie istniejących komponentów: TradingDashboard.tsx, MonitoringPanel.tsx, RiskMetricsPanel.tsx, StrategyPerformancePanel.tsx, TradingHistoryPanel.tsx
- ✅ Rozszerzanie funkcjonalności poprzez modyfikację istniejących komponentów
- ✅ Dodawanie nowych paneli TYLKO jako komponenty w `/dashboard/src/components/`
- ✅ Zachowanie stack technologicznego: React 18 + TypeScript 5.3 + Vite 5.0 + Recharts + Radix UI

### **KOMPLETNY DASHBOARD** (Enterprise-Grade):
...
**Komponenty** (~2000 LOC total):

```
/dashboard/
├── TradingDashboard.tsx          (402 LOC) - Main component z tabs
├── MonitoringPanel.tsx            (567 LOC) - KROK 5 System monitoring
├── RiskMetricsPanel.tsx           (9.6KB)   - VaR, Kelly Criterion analytics
├── StrategyPerformancePanel.tsx   (8.9KB)   - Per-strategy metrics
├── TradingHistoryPanel.tsx        (9.4KB)   - Trade history z filtrami
...
└── src/
    ...
    └── components/ui/             - Radix UI components (tabs, cards, alerts)
```

**Funkcjonalności**:

- ✅ Real-time WebSocket updates (ws://localhost:3001/ws)
- ✅ Multi-timeframe support (5m, 15m, 30m, 1h selectors)
- ✅ 4 Advanced Panels:
  - **Risk Metrics**: VaR (Parametric, Historical, Monte Carlo), Kelly Criterion
  - **Strategy Performance**: Per-strategy win rates, Sharpe ratios, metrics
  - **Trading History**: Recent trades z filtrami i sorting
  - **Monitoring**: System health, ML retrain stats, alerts (KROK 5)
- ✅ Interactive Charts: Portfolio value, equity curve, drawdown (Recharts)
- ✅ Alert System: Real-time alerts dla risk events, drawdown, ML degradation
- ✅ Health Checks: Component status monitoring

**API Integration** (Bot na port 3001):
...
GET /api/monitoring/summary              # Monitoring metrics
GET /api/analytics/daily                 # Daily analytics
GET /api/analytics/strategies            # Strategy performance
GET /api/analytics/portfolio/timeseries  # Portfolio timeseries
GET /api/analytics/risk/timeseries       # Risk timeseries
...
# Production build
cd dashboard && npm run build  # Output: /dashboard/dist
...
- Production: Static files z /dashboard/dist + Nginx/Caddy reverse proxy do Bot API (port 3001)

### **🚨 PAMIĘTAJ: DASHBOARD CLEANUP WYKONANY 26.12.2025**

**Usunięte (98+ plików)**:

- ❌ enterprise-dashboard.html, enterprise-dashboard-edit.html
- ❌ dashboard-server.js, unified_dashboard_server.js, simple_monitoring_system.js
- ❌ Wszystkie dashboardy z /src/ (30+ plików)
- ❌ Wszystkie dashboardy z /trading-bot/ (40+ plików)
- ❌ Stare logi dashboardów

**Zachowane (JEDYNY DASHBOARD)**:

- ✅ `/dashboard` - React Enterprise Dashboard (kompletny)

**🚫 NIGDY NIE TWÓRZ NOWYCH DASHBOARDÓW - TO JEST ROZKAZ!**
...
- **3.3 Auto-Retrain ML** ✅ **NOWE** (25.12.2025)
  ...
  - Weight Adjustment: Composite score = accuracy _ 0.4 + win_rate _ 0.3 + sharpe \* 0.3
...
## 🛠️ VSCode TOOLING & PRODUCTIVITY - DOSTĘPNE NARZĘDZIA

### **🔌 Zainstalowane Rozszerzenia (9):**

1. **Code Runner** (Ctrl+Alt+N) - Natychmiastowe uruchamianie kodu bez uruchamiania całego bota
2. **npm Intellisense** - Auto-complete dla importów pakietów i modułów
3. **Thunder Client** (⚡ icon) - REST API testing wewnątrz VSCode (alternatywa dla Postman)
4. **Test Explorer UI** - Wizualny interfejs do przeglądania i uruchamiania testów
5. **WakaTime** - Time tracking i analytics produktywności (automatyczne śledzenie czasu)
6. **Turbo Console Log** (Ctrl+Alt+L) - Szybkie dodawanie/usuwanie console.log dla debugowania
7. **Code Coverage** - Wizualizacja pokrycia kodu testami (kolorowe podświetlenie)
8. **JavaScript Booster** (Ctrl+.) - Zaawansowane refaktoringi i code actions

### **⚙️ VSCode Tasks (30+ dostępnych):**

**Uruchomienie**: `Ctrl+Shift+P` → "Tasks: Run Task" → wybierz z listy

**Kategorie:**

- **Build & Compile** (3): TypeScript build, type check, clean build
- **Testing** (3): run all tests, coverage report, watch mode
- **Bot Operations** (3): simulation mode, backtest mode, live mode
- **Monitoring & Health** (4): health check, portfolio status, ensemble ML status, Prometheus metrics
- **Development Utilities** (3): format code (Prettier), lint TypeScript, clean cache
- **Dependency Management** (3): install dependencies, update packages, security audit
- **VPS Deployment** (6): deploy dashboard, check bot status, connect VPS, view logs, restart bot, PM2 status

### **✂️ Custom Code Snippets (10):**

**Użycie**: Type prefix → Tab (autocomplete)

- `strategy` - Pełna klasa trading strategy z metodą generateSignal
- `mlpred` - ML prediction handler z try/catch
- `riskcheck` - Risk manager assessment z rejectionem
- `tryretry` - Retry logic z exponential backoff
- `ensreg` - Ensemble model registration (predict + updateOutcome)
- `portopt` - Portfolio optimization workflow
- `apiget` - Express API GET endpoint z error handling
- `sleep` - Async sleep utility function
- `logctx` - Logger z contextual information
- `features` - ML feature extraction method template

### **⚡ Thunder Client API Collection (7 requests):**

**Dostęp**: Click ⚡ icon w left sidebar → Collections → "Trading Bot API"

**Skonfigurowane Requesty:**

1. **Health Check** (GET /health) - Sprawdź status systemu
2. **Portfolio Status** (GET /api/portfolio) - Pozycje, cash, PnL
3. **Ensemble ML Status** (GET /api/ensemble/status) - Status modeli ML
4. **Trade History** (GET /api/trades?limit=50) - Historia transakcji
5. **Portfolio Optimization** (POST /api/portfolio/optimization) - Optymalizacja alokacji
6. **Backtest Validation** (POST /api/backtest/validate) - Walidacja strategii
7. **Prometheus Metrics** (GET /metrics) - Metryki systemowe

**Wszystkie requesty mają automated tests** (pm.test) dla walidacji odpowiedzi.

### **⌨️ Custom Keyboard Shortcuts:**

- `Ctrl+Shift+T` → Run All Tests
- `Ctrl+Shift+S` → Start Bot Simulation Mode
- `Ctrl+Shift+C` → TypeScript Type Check
- `Ctrl+Shift+H` → Health Check (API request)
- `Ctrl+Shift+P` → Portfolio Status (API request)
- `Ctrl+Shift+Delete` → Clean Build (remove dist/, cache)
- `Ctrl+Shift+F` → Format Code (Prettier)

### **🐛 Debug Configurations (8):**

**Uruchomienie**: `F5` lub Debug panel (`Ctrl+Shift+D`)

1. **🚀 Simulation Mode** (default, safe) - Mock data, bez realnych API calls
2. **🧪 Backtest Mode** - Historical data replay
3. **⚡ Live Mode (PRODUCTION)** - Real OKX API, wymaga ENABLE_REAL_TRADING=true
4. **🔧 Debug Current File** - Debuguj aktualnie otwarty plik .ts
5. **🧩 Debug SystemInitializer** - Focus na TIER 3 systems initialization
6. **🎯 Debug TradingEngine** - Focus na trading cycle execution
7. **🧠 Debug ML Integration** - Focus na ensemble predictions
8. **🔗 Attach to PM2** - Attach do działającego bota na VPS

**Breakpoint Locations** (AutonomousTradingBot.ts):

- Line 175: SystemInitializer initialization
- Line 343: Main trading cycle (KEY breakpoint)
- Line 346: ML retraining check
- Line 349: Portfolio rebalancing check

### **📖 Pełna Dokumentacja Toolingu:**

**[EXTENSIONS_GUIDE.md](../EXTENSIONS_GUIDE.md)** - Kompletny przewodnik praktyczny zawierający:

- 6 szczegółowych scenariuszy użycia (strategy development, API testing, refactoring, etc.)
- Real-world examples (bug hunting, API performance testing, coverage improvement)
- Daily workflow checklist (morning, during work, before commit, evening)
- Pro tips dla każdego rozszerzenia
- Keyboard shortcuts summary table

### **💡 PRAKTYCZNE UŻYCIE W CODZIENNEJ PRACY:**

**Szybkie Testowanie Algorytmów** (Code Runner):

```typescript
// Utwórz test-quick.ts, wpisz kod, naciśnij Ctrl+Alt+N
function kellyRatio(winRate: number, avgWin: number): number {
  return (winRate * avgWin - (1 - winRate)) / avgWin;
}
console.log(kellyRatio(0.6, 1.5)); // Instant result w OUTPUT
```

**Szybkie Debugowanie** (Turbo Console Log):

```typescript
// Zaznacz zmienną → Ctrl+Alt+L → automatycznie dodaje:
console.log("🚀 ~ executeTradingCycle ~ signal:", signal);
// Ctrl+Alt+D → usuń wszystkie logi przed commitem
```

**API Testing bez Postmana** (Thunder Client):

```
1. Start bot: Ctrl+Shift+S
2. Click ⚡ icon → Collections
3. Send request "Portfolio Status"
4. Automated tests validate response
5. Iterate without leaving VSCode
```

**Szybkie Generowanie Kodu** (Snippets):

```typescript
// Type: strategy + Tab → pełna klasa pojawi się automatycznie
export class NewStrategy extends AbstractStrategy {
  async generateSignal(candles, state) {
...
```bash
Ctrl+Shift+H  # Health check VPS bot
⚡ Thunder Client → Portfolio Status  # Sprawdź stan portfolio
WakaTime dashboard  # Sprawdź cele produktywności
```

**PODCZAS developmentu:**

```
1. Type snippet prefix → Tab (generuj kod)
2. npm Intellisense → Auto-import packages
3. Ctrl+Alt+N → Quick tests (Code Runner)
4. Ctrl+Alt+L → Debug logs (Turbo Console Log)
5. F5 → Full debugging with breakpoints
6. Ctrl+. → Quick fixes (JavaScript Booster)
```
...
Ctrl+Shift+T  # Run all tests
Code Coverage  # Visual check (green lines = covered)
Ctrl+Alt+D  # Remove all console.logs
Git commit
```

**🚨 WAŻNE: Wszystkie te narzędzia są już skonfigurowane i gotowe do użycia! Sprawdź [EXTENSIONS_GUIDE.md](../EXTENSIONS_GUIDE.md) dla szczegółowych przykładów.**
...
app.post("/api/backtest/validate", async (req, res) => {
  ...
  const backtest = await this.backtestEngine.runBacktest(strategy, marketData);
