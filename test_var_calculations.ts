...
import { IntegratedPerformanceManager } from './trading-bot/core/analysis/integrated_performance_manager';
import { Logger } from './trading-bot/infrastructure/logging/logger';

async function testVarCalculations(): Promise<void> {
  const logger = new Logger('VarTest');
  logger.info('🧪 Starting VaR calculations test...');

  try {
    // Initialize components
    const performanceTracker = new PerformanceTracker();
    const enterpriseAnalyzer = new EnterprisePerformanceAnalyzer(performanceTracker);
    ...
    for (let i = 0; i < sampleTrades.length; i++) {
      ...
      const tradeId = performanceTracker.recordTrade(
        'BTCUSDT',
        'BUY',
        0.1,
        50000,
