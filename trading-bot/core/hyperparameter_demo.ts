...
import {
    ...
    TransformationType,
    SamplingStrategy
} from './hyperparameter_space';

import {
    initializePredefinedSpaces,
    SpaceBuilder,
    MA_CROSSOVER_SPACE,
    RSI_STRATEGY_SPACE,
    MULTI_TIMEFRAME_SPACE
} from './predefined_spaces';
...
async function demonstrateHyperparameterSpaces() {
    ...
    try {
        ...
        const allSpaces = hyperparameterSpaceManager.getAllSpaces();
        console.log(`✅ Załadowano ${allSpaces.length} przestrzeni:`);
        
        allSpaces.forEach(space => {
            const stats = hyperparameterSpaceManager.getSpaceStatistics(space.name);
    ...
    for (const spaceName of spacesToAnalyze) {
        ...
        const space = hyperparameterSpaceManager.getSpace(spaceName);
        if (!space) {
        ...
        space.parameters.forEach((param, index) => {
            const typeInfo = param.type === ParameterType.INTEGER || param.type === ParameterType.FLOAT
                ? `[${param.min}-${param.max}]`
                : param.type === ParameterType.CATEGORICAL
                ? `{${param.choices?.length} opcji}`
                : `{${param.type}}`;
            
            const transformInfo = param.transformation && param.transformation !== TransformationType.NONE
        ...
        // Statystyki
        const stats = hyperparameterSpaceManager.getSpaceStatistics(spaceName);
        if (stats) {
            console.log(`\n  📊 Statystyki:`);
    ...
    for (const strategy of samplingStrategies) {
        ...
        try {
            ...
            const samples = hyperparameterSpaceManager.sampleMultiple('ma_crossover', 5, strategy);
    ...
    try {
        ...
        const conditionalSamples = hyperparameterSpaceManager.sampleMultiple('multi_timeframe_strategy', 10);
    ...
    try {
        const customSpace = new SpaceBuilder('momentum_strategy', 'MomentumStrategy')
            ...
            .addFloatParam('momentum_threshold', 0.01, 0.1, {
                ...
                samplingStrategy: SamplingStrategy.LOG_UNIFORM,
                importance: 0.85,
                tags: ['threshold']
            })
        ...
        const customSpaceId = hyperparameterSpaceManager.createSpace(customSpace);
        console.log(`✅ Utworzono niestandardową przestrzeń: ${customSpaceId}`);

        // Test próbkowania z niestandardowej przestrzeni
        const customSamples = hyperparameterSpaceManager.sampleMultiple(customSpaceId, 3);
    ...
    try {
        ...
        const rsiSamples = hyperparameterSpaceManager.sampleMultiple('rsi_strategy', 5);
    ...
    const finalSpaces = hyperparameterSpaceManager.getAllSpaces();
...
async function performanceTest() {
    ...
    for (const testCase of testCases) {
        ...
        try {
            const samples = hyperparameterSpaceManager.sampleMultiple(testCase.space, testCase.samples);
...
export { demonstrateHyperparameterSpaces, performanceTest };
