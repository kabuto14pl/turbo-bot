...
import {
    ...
    MonitoringConfig,
    AlertConfig
} from './monitoring_adaptation_system';
...
class DemoDataGenerator {
    ...
    private marketRegime: 'bull' | 'bear' | 'sideways' | 'volatile' | 'crisis' = 'bull';
    private volatilityLevel: number = 0.02;
    private trendStrength: number = 0.05;

    /**
     * Generuje syntetyczne metryki wydajności dla strategii
     */
    generatePerformanceMetrics(strategyId: string, degradationFactor: number = 0): PerformanceMetrics {
...
export class MonitoringAdaptationDemo {
    private system: MonitoringAndAdaptationSystem;
    ...
    constructor() {
        this.system = new MonitoringAndAdaptationSystem(DEMO_CONFIG);
     ...
     */
    private async demoBasicMonitoring(): Promise<void> {
     ...
     */
    private async demoPerformanceDegradation(): Promise<void> {
     ...
     */
    private async demoAutomaticAdaptation(): Promise<void> {
    ...
    private async demoMarketRegimeAdaptation(): Promise<void> {
        ...
        const regimes: ('bull' | 'bear' | 'sideways' | 'volatile' | 'crisis')[] = 
            ['bull', 'bear', 'volatile', 'crisis', 'sideways'];
        ...
        for (const regime of regimes) {
            ...
            for (const strategyId of this.demoStrategies) {
                ...
                if (regime === 'crisis') degradationForRegime = 0.4;
                else if (regime === 'volatile') degradationForRegime = 0.2;
                else if (regime === 'bear') degradationForRegime = 0.1;
     ...
     */
    private async demoHealthReports(): Promise<void> {
     ...
     */
    private async demoAlertSystem(): Promise<void> {
     ...
     */
    private async demoContinuousMonitoring(): Promise<void> {
