...
import { MarketData, BacktestResult, StrategyParameters } from './advanced_backtesting';
import { GeneratedStrategy, AutomaticStrategyGenerator } from './automatic_strategy_generator';
...
export interface PerformanceMetrics {
    ...
    marketRegime: 'bull' | 'bear' | 'sideways' | 'volatile' | 'crisis';
    marketVolatility: number;
    marketTrend: number;
    correlation: number;
    
    // Degradation Indicators
    performanceDegradation: number; // 0-1 scale
...
export interface MarketCondition {
    ...
    // Technical indicators for market state
    vix: number;
    rsi: number;
    macd: number;
...
export interface AdaptationAction {
    timestamp: number;
    strategyId: string;
    actionType: 'parameter_adjustment' | 'weight_rebalancing' | 'strategy_replacement' | 'emergency_stop';
    reason: string;
    confidence: number;
...
export interface AlertConfig {
    ...
    sharpeDeclineThreshold: number;
    returnDeclineThreshold: number;
    
    // Risk alerts
    volatilityIncreaseThreshold: number;
    correlationChangeThreshold: number;
    var95ExceedanceThreshold: number;
...
export interface StrategyHealthReport {
    ...
    // Recommendations
    recommendations: {
        action: 'continue' | 'adjust' | 'replace' | 'stop';
        confidence: number;
        reason: string;
...
export class PerformanceTracker extends EventEmitter {
     ...
     */
    addMetrics(strategyId: string, metrics: PerformanceMetrics): void {
     ...
     */
    calculatePerformanceDegradation(strategyId: string): number {
     ...
     */
    private checkAlerts(strategyId: string, metrics: PerformanceMetrics): void {
    ...
    private calculateAlertSeverity(alerts: string[]): 'low' | 'medium' | 'high' | 'critical' {
        if (alerts.some(alert => alert.includes('Max drawdown') && alert.includes('30'))) return 'critical';
        if (alerts.some(alert => alert.includes('VaR') || alert.includes('Volatility spike'))) return 'high';
     ...
     */
    getLatestMetrics(strategyId: string): PerformanceMetrics | null {
        const metrics = this.metrics.get(strategyId);
     ...
     */
    getMetricsHistory(strategyId: string, limit?: number): PerformanceMetrics[] {
        const metrics = this.metrics.get(strategyId) || [];
...
export class AdaptiveParameterOptimizer extends EventEmitter {
    ...
    constructor(tracker: PerformanceTracker, config: MonitoringConfig) {
        super();
     ...
     */
    async checkAndAdapt(strategyId: string, currentParameters: StrategyParameters): Promise<AdaptationAction | null> {
        if (!this.config.adaptationEnabled) return null;
     ...
     */
    private async analyzeAdaptationNeed(strategyId: string): Promise<{
        score: number;
        reasons: string[];
        urgency: 'low' | 'medium' | 'high' | 'critical';
        ...
        if (adaptationScore > 0.8) urgency = 'critical';
        else if (adaptationScore > 0.6) urgency = 'high';
        else if (adaptationScore > 0.4) urgency = 'medium';
    ...
    private async performAdaptation(
        ...
        if (!metrics || !marketCondition) return null;
     ...
     */
    private generateAdaptedParameters(
    ...
    private determineActionType(adaptationNeed: any): AdaptationAction['actionType'] {
        if (adaptationNeed.urgency === 'critical') return 'emergency_stop';
        if (adaptationNeed.score > 0.7) return 'strategy_replacement';
        if (adaptationNeed.score > 0.5) return 'weight_rebalancing';
        return 'parameter_adjustment';
    }

    private calculateAdaptationConfidence(adaptationNeed: any): number {
        return Math.min(adaptationNeed.score * 1.2, 1.0);
    }

    private estimateImprovement(adaptationNeed: any): number {
        return adaptationNeed.score * 0.1; // Estimated 10% improvement per adaptation score point
    }

    private calculateRegimeStability(regimes: string[]): number {
        if (regimes.length < 2) return 1.0;
    ...
    private getTodayAdaptations(strategyId: string): number {
        const today = new Date().toDateString();
    ...
    private recordAdaptation(strategyId: string, adaptation: AdaptationAction): void {
     ...
     */
    getAdaptationHistory(strategyId: string): AdaptationAction[] {
...
export class StrategyHealthAnalyzer extends EventEmitter {
    private tracker: PerformanceTracker;
    private optimizer: AdaptiveParameterOptimizer;

    constructor(tracker: PerformanceTracker, optimizer: AdaptiveParameterOptimizer) {
        super();
        this.tracker = tracker;
     ...
     */
    async generateHealthReport(strategyId: string): Promise<StrategyHealthReport> {
     ...
     */
    private generateDefaultHealthReport(strategyId: string, latestMetrics: PerformanceMetrics | null): StrategyHealthReport {
        const now = Date.now();
    ...
    private calculateAdaptabilityScore(adaptationHistory: AdaptationAction[]): number {
        if (adaptationHistory.length === 0) return 70; // Neutral score for no adaptations
    ...
    private analyzeTrend(metrics: PerformanceMetrics[]): { 
        ...
        if (overallChange > 0.1) direction = 'improving';
        else if (overallChange < -0.2) direction = 'critical';
        else if (overallChange < -0.05) direction = 'declining';
    ...
    private async generateRecommendations(
        ...
        healthScore: number, 
        trend: any
    ): Promise<StrategyHealthReport['recommendations']> {
        const recommendations: StrategyHealthReport['recommendations'] = [];
    ...
    private generatePredictions(metrics: PerformanceMetrics[]): StrategyHealthReport['predictedPerformance'] {
        const recentMetrics = metrics.slice(-10);
        const avgReturn = recentMetrics.reduce((sum, m) => sum + m.totalReturn, 0) / recentMetrics.length;
...
export class MonitoringAndAdaptationSystem extends EventEmitter {
    private tracker: PerformanceTracker;
    private optimizer: AdaptiveParameterOptimizer;
    private analyzer: StrategyHealthAnalyzer;
    private config: MonitoringConfig;
     ...
     */
    updateStrategyMetrics(strategyId: string, metrics: PerformanceMetrics): void {
     ...
     */
    async generateStrategyReport(strategyId: string): Promise<StrategyHealthReport> {
     ...
     */
    async saveReport(report: StrategyHealthReport, outputPath: string): Promise<void> {
        const reportData = {
...
export default MonitoringAndAdaptationSystem;
