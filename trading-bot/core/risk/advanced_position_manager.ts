...
import { AdvancedStopLossManager, TrailingStopConfig, PositionData, StopLossData } from './advanced_stop_loss';
...
export interface PositionManagerConfig {
    ...
    enablePortfolioHedging: boolean;
    rebalanceThreshold: number;
}

export interface ActivePosition extends PositionData {
    strategy: string;
...
export interface PortfolioMetrics {
    ...
    riskDistribution: { [strategy: string]: number };
    correlationRisk: number;
    portfolioHeat: number; // 0-1 scale of portfolio stress
}

export class AdvancedPositionManager {
    private readonly logger: Logger;
    private readonly config: PositionManagerConfig;
    private readonly stopLossManager: AdvancedStopLossManager;
    private activePositions: Map<string, ActivePosition> = new Map();
    private positionHistory: ActivePosition[] = [];
    ...
    constructor(
        ...
        this.config = config;
        this.logger = logger;
        this.stopLossManager = new AdvancedStopLossManager(stopLossConfig, logger);
    }
     ...
     */
    async updatePositions(marketData: { [symbol: string]: number }): Promise<void> {
     ...
     */
    private canOpenPosition(riskPercent: number, strategy: string): boolean {
     ...
     */
    private checkCorrelationLimits(symbol: string, strategy: string): boolean {
        const correlationGroup = this.getCorrelationGroup(symbol);
        if (!correlationGroup) return true;
     ...
     */
    private getCorrelationGroup(symbol: string): string | undefined {
    ...
    private calculateVolatility(symbol: string): number {
        ...
        return volatilityMap[symbol as keyof typeof volatilityMap] || 0.05;
    }
    ...
    private async checkRebalanceNeed(): Promise<void> {
        ...
        if (hoursSinceLastRebalance < 4) return; // Rebalansuj maksymalnie co 4 godziny
    ...
    getPortfolioMetrics(): PortfolioMetrics {
        ...
        const riskDistribution: { [strategy: string]: number } = {};
    ...
    private calculateCorrelationRisk(): number {
        const groupRisks: { [group: string]: number } = {};
     ...
     */
    getPositionHistory(limit: number = 100): ActivePosition[] {
