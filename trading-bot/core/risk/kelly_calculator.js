...
exports.KellyCalculator = void 0;
class KellyCalculator {
    ...
    calculateSize(signal, state, atr) {
        // Oblicz podstawowe parametry Kelly
        const { winRate, avgWin, avgLoss } = this.calculateHistoricalStats(signal.metadata?.strategy || 'unknown');
        ...
        const kellyFraction = this.calculateKellyFraction(effectiveWinRate, effectiveAvgWin, effectiveAvgLoss);
        // Dostosuj frakcję
        const adjustedFraction = this.adjustKellyFraction(kellyFraction, signal, state);
        // Oblicz wielkość pozycji
        const positionSize = this.calculatePositionSize(adjustedFraction, state.equity, signal.price, atr);
    ...
    addTradeResult(strategy, pnl, risk) {
    ...
    calculateHistoricalStats(strategy) {
        const history = this.tradeHistory.get(strategy);
    ...
    adjustKellyFraction(kellyFraction, signal, state) {
    ...
    calculatePositionSize(fraction, equity, price, atr) {
...
exports.KellyCalculator = KellyCalculator;
