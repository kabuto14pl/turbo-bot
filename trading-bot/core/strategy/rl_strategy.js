...
class RLStrategy {
    constructor(config) {
        ...
        this.isEnabled = true;
        this.config = config;
        this.logger = new logger_1.Logger('RLStrategy');
    ...
    async execute(candle, indicators, botState) {
        if (!this.isEnabled) {
     ...
     */
    convertToMarketState(candle, indicators, botState) {
     ...
     */
    convertToStrategySignal(rlAction, candle) {
        ...
        const mappedType = rlAction.action_type === 'BUY' ? 'ENTER_LONG' : 'ENTER_SHORT';
        const signal = {
            price: candle.close,
            type: mappedType,
     ...
     */
    async learnFromTradeResult(tradeResult, isPositionClosed) {
        ...
        try {
            ...
            const dummyState = this.createDummyState(); // Would use actual state in production
