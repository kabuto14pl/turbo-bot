...
exports.MultiTimeframeStrategyAnalyzer = void 0;
const events_1 = require("events");
const logger_1 = require("../utils/logger");
// ============================================================================
// 🏗️ MULTI-TIMEFRAME STRATEGY ANALYZER
// ============================================================================
class MultiTimeframeStrategyAnalyzer extends events_1.EventEmitter {
    constructor(logger) {
        super();
        this.timeframeConfigs = new Map();
        this.timeframeSignals = new Map();
    ...
    // ========================================================================
    configureTimeframe(timeframe, config) {
    ...
    // 🎯 SIGNAL ANALYSIS
    // ========================================================================
    async analyzeMultiTimeframe(strategies, state) {
        const timeframeSignals = [];
        // Generate signals for each enabled timeframe
        for (const [timeframe, config] of this.timeframeConfigs) {
            if (!config.enabled)
                continue;
            const tfSignals = await this.generateTimeframeSignals(strategies, state, timeframe, config);
        ...
        const recommendations = this.generateRecommendations(overallSignal, timeframeSignals, conflictScore, trendAlignment);
        const analysis = {
            overallSignal,
            confidence,
            timeframeSignals,
    ...
    async generateTimeframeSignals(strategies, state, timeframe, config) {
        const signals = [];
    ...
    analyzeTimeframeTrend(state, timeframe) {
    ...
    calculateTrendAlignment(signals) {
        if (signals.length === 0)
            return 0;
    ...
    calculateStrengthScore(signals) {
        if (signals.length === 0)
            return 0;
    ...
    generateOverallSignal(signals) {
        ...
        if (buyScore > sellScore + threshold)
            return 'BUY';
        if (sellScore > buyScore + threshold)
            return 'SELL';
        return 'HOLD';
    }
    calculateOverallConfidence(signals, conflictScore, trendAlignment) {
        if (signals.length === 0)
            return 0;
    ...
    generateRecommendations(overallSignal, signals, conflictScore, trendAlignment) {
        const reasoning = [];
    ...
    determineTrend(signal, state) {
        if (signal.type.includes('ENTER_LONG'))
            return 'bullish';
        if (signal.type.includes('ENTER_SHORT'))
            return 'bearish';
        return 'neutral';
    }
    // ========================================================================
...
exports.MultiTimeframeStrategyAnalyzer = MultiTimeframeStrategyAnalyzer;
exports.default = MultiTimeframeStrategyAnalyzer;
