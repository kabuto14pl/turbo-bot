...
export class PairsTradingStrategy {
        ...
        try {
            ...
            return {
                ...
                metadata: {
                    ...
                    timestamp: Date.now(),
                    activePairs: this.pairData.size,
                    cointegrated: this.cointegrationResults.size,
                    ...metrics
                }
            };
            
        } catch (error) {
            throw new Error(`Pairs trading strategy error: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    private async generatePairsSignals(): Promise<PairsSignal[]> {
        ...
        const pairKeys = Array.from(this.pairData.keys());
            ...
            try {
                ...
                if (Math.abs(currentZScore) >= this.entryThreshold) {
                    const signal: PairsSignal = {
                        type: currentZScore > 0 ? 'SELL' : 'BUY', // Sell when spread is high, buy when low
    ...
    private calculateStrategyMetrics(signals: PairsSignal[]): any {
        ...
        const avgConfidence = signals.reduce((sum, s) => sum + s.confidence, 0) / Math.max(totalSignals, 1);
