 ...
 */
import { Strategy, StrategyConfig, BotState, StrategySignal, Indicators } from '../types/strategy';
import { EnhancedStrategySignal } from '../types/enhanced_strategy_signal';
import { Logger } from '../../infrastructure/logging/logger';
import { IndicatorSet } from '../types/indicator_set';

type TimeframeKey = 'm15' | 'h1' | 'h4' | 'd1';
...
export abstract class BaseStrategy implements Strategy {
    ...
    // Implementation of Strategy interface generateSignal method
     ...
     */
    protected processSentiment(state: BotState): {
        ...
        const signalConfidence = Math.min(1.0, 
            0.5 + (sentiment.confidence * 0.3) + (sentiment.signalConfidence * 0.2)
        );
     ...
     */
    protected enhanceSignalWithSentiment(
        ...
        let enhancedConfidence = Math.min(1.0, 
            baseSignal.confidence * sentimentData.sentimentMultiplier * sentimentData.signalConfidence
        );
        ...
        if (state.sentiment.tradingSignal === 'strong_buy' && baseSignal.type === 'ENTER_LONG') {
            ...
            (state.sentiment.tradingSignal === 'sell' || state.sentiment.tradingSignal === 'strong_sell') && 
            (baseSignal.type === 'ENTER_LONG')
        ) {
            enhancedConfidence *= 0.7; // Reduce confidence for conflicting signals
            conflictDetected = true;
        } else if (
            (state.sentiment.tradingSignal === 'buy' || state.sentiment.tradingSignal === 'strong_buy') && 
     ...
     */
    async executeWithSentiment(state: BotState): Promise<EnhancedStrategySignal> {
    ...
    recordSentimentTrade(
        ...
        state: BotState,
        performanceTracker?: any // PerformanceTracker instance
    ): void {
        ...
        if (!sentimentData.shouldTrade && signal.type !== 'HOLD') {
            performanceTracker.recordBlockedTrade(
                'BTCUSDT', // Default symbol, could be passed as parameter
                signal.type === 'ENTER_LONG' || signal.type === 'EXIT_SHORT' ? 'BUY' : 'SELL',
                signal.size || 0.1,
                state.prices.m15?.close || 0,
                state.sentiment?.overall || 0.5,
                state.sentiment?.tradingSignal || 'low confidence',
                this.name
            );
     ...
     */
    async executeWithSentimentTracking(
        state: BotState, 
    ...
    protected hasIndicator(indicators: IndicatorSet, indicator: string): boolean {
        switch (indicator) {
    ...
    protected calculateConfidence(
        ...
        // Oblicz ważoną sumę
        const confidence = (
            normalizedPrimary * weights.primary +
