 ...
 */
class PairsTradingStrategy extends events_1.EventEmitter {
    constructor(config) {
        super();
        this.pairDatabase = new Map();
        this.correlationThreshold = 0.8;
        this.cointegrationThreshold = 0.05;
        this.zScoreEntry = 2.0;
        this.zScoreExit = 0.5;
        this.lookbackPeriod = 252; // Trading days
        this.minTradingPeriod = 20;
        if (config) {
    ...
    async execute(data) {
        try {
            ...
            this.emit('error', { type: types_1.ErrorType.STRATEGY_ERROR, message: errorMsg });
            throw error;
        }
    }
    /**
     ...
     */
    async identifyTradingPairs(data) {
    ...
    async testCointegration(pairs) {
        ...
        for (const pair of pairs) {
            try {
                ...
                const { hedgeRatio, pValue } = this.performCointegrationTest(symbol1Data.prices1, symbol2Data.prices1);
                results.push({
     ...
     */
    performCointegrationTest(prices1, prices2) {
     ...
     */
    async generatePairsSignals(cointegrated, data) {
        const signals = [];
        for (const coint of cointegrated) {
            ...
            if (spread.length < this.minTradingPeriod)
                continue;
            // Calculate rolling statistics
            const spreadMean = spread.reduce((a, b) => a + b, 0) / spread.length;
            const spreadStd = Math.sqrt(spread.reduce((sum, s) => sum + Math.pow(s - spreadMean, 2), 0) / spread.length);
            if (spreadStd === 0)
                continue;
            // Current z-score
     ...
     */
    calculateSpread(data, hedgeRatio) {
     ...
     */
    calculatePerformance(signals, data) {
        if (signals.length === 0) {
            return {
                totalReturn: 0,
                sharpeRatio: 0,
        ...
        return {
            totalReturn,
            sharpeRatio,
            maxDrawdown: Math.min(...returns.map((_, i) => returns.slice(0, i + 1).reduce((a, b) => a + b, 0))),
            winRate,
...
exports.default = PairsTradingStrategy;
