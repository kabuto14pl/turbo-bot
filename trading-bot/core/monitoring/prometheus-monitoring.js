...
class PrometheusMonitoring extends events_1.EventEmitter {
    ...
    setupExpressApp() {
        ...
        this.app.post('/webhook/alerts', express_1.default.json(), (req, res) => {
            try {
                console.log('📨 Received AlertManager webhook:', req.body);
        ...
        this.app.post('/webhook/alerts/critical', express_1.default.json(), (req, res) => {
            try {
                console.log('🚨 CRITICAL AlertManager webhook:', req.body);
                this.handleAlertManagerWebhook(req.body, 'critical');
                res.status(200).json({ status: 'received' });
            }
            catch (error) {
                console.error('❌ Error handling critical webhook:', error);
                res.status(500).json({ error: 'Error processing webhook' });
            }
        });
        this.app.post('/webhook/alerts/emergency', express_1.default.json(), (req, res) => {
            try {
                console.log('🆘 EMERGENCY AlertManager webhook:', req.body);
                this.handleAlertManagerWebhook(req.body, 'emergency');
                res.status(200).json({ status: 'received' });
            }
            catch (error) {
                console.error('❌ Error handling emergency webhook:', error);
                res.status(500).json({ error: 'Error processing webhook' });
            }
        });
        this.app.post('/webhook/alerts/risk', express_1.default.json(), (req, res) => {
            try {
                console.log('⚠️ RISK AlertManager webhook:', req.body);
                this.handleAlertManagerWebhook(req.body, 'risk');
                res.status(200).json({ status: 'received' });
            }
            catch (error) {
                console.error('❌ Error handling risk webhook:', error);
                res.status(500).json({ error: 'Error processing webhook' });
            }
        });
        // 🧪 TEST TRADE ENDPOINT - Generate simulated trade for testing
        this.app.post('/test/trade', express_1.default.json(), (req, res) => {
            try {
                console.log('🧪 TEST TRADE requested');
                // Generate test trade
                const testTrade = {
                ...
                console.log(`🧪 TEST TRADE executed: ${testTrade.side} ${testTrade.amount} ${testTrade.symbol} @ $${testTrade.price}`);
                res.status(200).json({
                    status: 'executed',
                    trade: testTrade,
        ...
        return new Promise((resolve, reject) => {
            try {
                this.server = this.app.listen(this.port, '0.0.0.0', () => {
                    console.log(`📊 Prometheus monitoring server started on port ${this.port}`);
    ...
    async stop() {
        return new Promise((resolve) => {
            if (this.server) {
                this.server.close(() => {
                    console.log('📊 Prometheus monitoring server stopped');
                    this.emit('stopped');
                    resolve();
                });
            }
    ...
    handleAlertManagerWebhook(alertData, alertType) {
        try {
            ...
            console.log(`📊 Processing ${alerts.length} alerts from AlertManager (type: ${alertType || 'general'})`);
    ...
    processPrometheusAlert(alert, alertType) {
        ...
        console.log(`🔔 Alert: ${alertInfo.name} (${alertInfo.severity}) - ${alertInfo.description}`);
        // Record alert metric
