...
import {
    ...
    OptimizationHistory
} from './periodic_reoptimization_system';

import { MonitoringAndAdaptationSystem, MonitoringConfig } from './monitoring_adaptation_system';
import { AutomaticStrategyGenerator, StrategyGenerationConfig } from './automatic_strategy_generator';
import { AdvancedBacktestingSystem, BacktestConfig } from './advanced_backtesting';

// ============================================================================
// DEMO CONFIGURATION
// ============================================================================

const REOPTIMIZATION_CONFIG: ReoptimizationConfig = {
    enabled: true,
...
class ReoptimizationDemoDataGenerator {
    ...
    private strategiesData: Map<string, {
     ...
     */
    initializeStrategy(strategyId: string): void {
     ...
     */
    simulatePerformanceDegradation(strategyId: string, baseMetrics: any): any {
        const strategyData = this.strategiesData.get(strategyId);
        if (!strategyData) return baseMetrics;
     ...
     */
    simulateReoptimization(strategyId: string): void {
        const strategyData = this.strategiesData.get(strategyId);
    ...
    getStrategyData(strategyId: string) {
...
export class PeriodicReoptimizationDemo {
    private reoptimizationSystem: PeriodicReoptimizationSystem;
    private monitoringSystem: MonitoringAndAdaptationSystem;
    private dataGenerator: ReoptimizationDemoDataGenerator;
    
    private demoStrategies: string[] = [
    ...
    constructor() {
        ...
        this.monitoringSystem = new MonitoringAndAdaptationSystem(MONITORING_CONFIG);
        
        // Mock strategy generator and backtesting engine
        const mockStrategyGenerator = {} as AutomaticStrategyGenerator;
        const mockBacktestingEngine = {} as AdvancedBacktestingSystem;
        
        // Initialize reoptimization system
        this.reoptimizationSystem = new PeriodicReoptimizationSystem(
     ...
     */
    private async demoSystemSetup(): Promise<void> {
     ...
     */
    private async demoTriggerEngine(): Promise<void> {
        ...
        for (const scenario of scenarios) {
            ...
            const mockMetrics = {
                ...
                marketRegime: 'bull' as const, marketVolatility: 0, marketTrend: 0, correlation: 0,
                parameterDrift: 0, stabilityScore: 0, confidenceLevel: 0
            };

            // Test trigger response
            const triggerEngine = (this.reoptimizationSystem as any).triggerEngine;
            const triggerResult = await triggerEngine.checkReoptimizationNeed(
                this.demoStrategies[0],
                mockMetrics,
                this.dataGenerator.getCurrentTime() - 24 * 60 * 60 * 1000 // 1 day ago
            );
     ...
     */
    private async demoScheduledReoptimizations(): Promise<void> {
        ...
        const requestId = this.reoptimizationSystem.requestManualReoptimization(
            this.demoStrategies[0],
            'medium',
            'parameters'
        );
     ...
     */
    private async demoPerformanceBasedReoptimization(): Promise<void> {
        ...
        for (let period = 1; period <= 5; period++) {
            ...
            const degradedMetrics = this.dataGenerator.simulatePerformanceDegradation(
                targetStrategy,
                this.createMockMetrics(targetStrategy, degradationLevel)
            );
     ...
     */
    private async demoMarketRegimeAdaptation(): Promise<void> {
        console.log('🌊 DEMO 5: Adaptacja do Zmiany Reżimu Rynkowego');
        console.log('-'.repeat(60));

        const regimeSequence = ['bull', 'volatile', 'bear', 'crisis', 'sideways'] as const;
        ...
        for (const regime of regimeSequence) {
            ...
            for (const strategyId of this.demoStrategies) {
                ...
                if (regime === 'crisis') regimeDegradation = 0.25;
                else if (regime === 'volatile') regimeDegradation = 0.15;
                else if (regime === 'bear') regimeDegradation = 0.10;
     ...
     */
    private async demoOptimizationHistory(): Promise<void> {
     ...
     */
    private async demoLongTermAnalysis(): Promise<void> {
     ...
     */
    private async demoConcurrentOptimizations(): Promise<void> {
        ...
        const requests = [
            { strategy: this.demoStrategies[0], urgency: 'critical' as const, scope: 'full_strategy' as const },
            { strategy: this.demoStrategies[1], urgency: 'high' as const, scope: 'parameters' as const },
            { strategy: this.demoStrategies[2], urgency: 'medium' as const, scope: 'parameters' as const },
            { strategy: this.demoStrategies[3], urgency: 'low' as const, scope: 'parameters' as const },
            { strategy: this.demoStrategies[4], urgency: 'medium' as const, scope: 'ensemble' as const }
        ];
     ...
     */
    private createMockMetrics(strategyId: string, degradationFactor: number = 0): any {
