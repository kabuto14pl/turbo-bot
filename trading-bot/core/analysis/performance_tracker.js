...
class PerformanceTracker extends events_1.EventEmitter {
    ...
    recordTrade(symbol, side, quantity, price, strategy) {
    ...
    closeTrade(tradeId, exitPrice, fees = 0) {
    ...
    addSentimentData(tradeId, entrysentiment, exitSentiment) {
        const trade = this.trades.find(t => t.id === tradeId);
        if (trade) {
            trade.sentiment = {
    ...
    calculateSentimentMetrics() {
        ...
        const averageBlockedSentiment = totalBlockedTrades > 0
            ...
            : 0;
        const sentimentStrategies = new Map();
     ...
     */
    calculateRealSentimentAccuracy() {
        // Get all completed trades with sentiment data
        const completedTradesWithSentiment = this.trades.filter(trade => trade.exitPrice !== undefined &&
            trade.sentiment &&
            trade.pnl !== undefined);
     ...
     */
    estimateSentimentAccuracyFromBlockedTrades() {
        ...
        const extremeSentimentBlocks = this.blockedTrades.filter(trade => Math.abs(trade.sentiment?.entryScore || 0) > 0.7).length;
     ...
     */
    calculateSentimentPredictionErrors() {
        const completedTradesWithSentiment = this.trades.filter(trade => trade.exitPrice !== undefined &&
            trade.sentiment &&
            trade.pnl !== undefined);
        if (completedTradesWithSentiment.length === 0) {
    ...
    getSentimentPerformanceReport() {
        ...
        const highSentimentTrades = completedTrades.filter(t => t.sentiment && Math.abs(t.sentiment.entryScore) > 0.7);
     ...
     */
    calculateComprehensiveSentimentAnalysis(completedTrades) {
     ...
     */
    calculateRealSentimentSavedLosses(completedTrades) {
     ...
     */
    estimateBlockedTradeLoss(blockedTrade) {
        // Find similar executed trades with similar sentiment scores
        const similarTrades = this.trades.filter(trade => trade.symbol === blockedTrade.symbol &&
        ...
        const strategyTrades = this.trades.filter(trade => trade.strategy === blockedTrade.strategy &&
            trade.exitPrice !== undefined &&
            (trade.pnl || 0) < 0);
        if (strategyTrades.length > 0) {
            const strategyAverageLoss = Math.abs(strategyTrades.reduce((sum, t) => sum + (t.pnl || 0), 0) / strategyTrades.length);
     ...
     */
    calculateStatisticalSentimentSavedLosses(completedTrades) {
        const tradesWithSentiment = completedTrades.filter(t => t.sentiment && t.pnl !== undefined);
        if (tradesWithSentiment.length === 0)
            return 0;
        // Separate trades by sentiment score ranges
        const veryNegativeSentimentTrades = tradesWithSentiment.filter(t => (t.sentiment?.entryScore || 0) < -0.7);
    ...
    calculateSentimentPredictionQuality(trades) {
        ...
        for (const trade of trades) {
            ...
            if (sentimentPredictedProfit && actualProfit)
                truePositives++;
            else if (sentimentPredictedProfit && !actualProfit)
                falsePositives++;
            else if (!sentimentPredictedProfit && !actualProfit)
                trueNegatives++;
            else if (!sentimentPredictedProfit && actualProfit)
    ...
    calculateSentimentRiskReduction(trades) {
    ...
    calculateTradeVolatility(trades) {
        if (trades.length < 2)
            return 0;
    ...
    generateSentimentRecommendations(sentimentMetrics, analysis) {
     ...
     */
    getPerformanceForPeriod(period) {
     ...
     */
    analyzePerformance(metrics, period) {
        if (metrics.length === 0) {
     ...
     */
    calculateEnhancedSharpeRatio(returns) {
        if (returns.length === 0)
            return 0;
     ...
     */
    calculateTrendConfidence(metrics) {
        if (metrics.length < 5)
            return 0.5;
     ...
     */
    generateRecommendations(summary, trend) {
        const recommendations = [];
    ...
    checkAlertConditions(metrics) {
        ...
        if (drawdown > this.config.alertThresholds.drawdownPercent) {
            this.createAlert('DRAWDOWN_THRESHOLD', 'HIGH', `Portfolio drawdown exceeded threshold: ${drawdown.toFixed(2)}%`, metrics);
        }
        // System performance alert
        if (metrics.system?.latency && metrics.system.latency > this.config.alertThresholds.latencyMs) {
            this.createAlert('SYSTEM_ISSUE', 'MEDIUM', `High latency detected: ${metrics.system.latency}ms`, metrics);
        }
        // Error rate alert
        if (metrics.system?.errorRate && metrics.system.errorRate > this.config.alertThresholds.errorRate) {
            this.createAlert('SYSTEM_ISSUE', 'HIGH', `High error rate detected: ${metrics.system.errorRate}%`, metrics);
        }
    }
    /**
     ...
     */
    calculateSentimentDataQuality(trades) {
        if (trades.length === 0)
            return 0;
        let qualityScore = 0;
        const maxScore = 5;
        // 1. Data completeness (20%)
        const completenessScore = trades.filter(t => t.sentiment &&
            t.sentiment.entryScore !== undefined &&
            t.pnl !== undefined).length / trades.length;
