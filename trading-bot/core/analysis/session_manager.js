...
class SessionManager {
    ...
    getCurrentSession(timestamp = Date.now()) {
    ...
    getSessionVolatilityMultiplier(timestamp = Date.now()) {
        const session = this.getCurrentSession(timestamp);
        if (!session)
            return 1.0;
        return session.volatilityProfile.typical;
    }
    isActiveSession(timestamp = Date.now(), sessionName) {
        if (sessionName) {
        ...
        return Object.values(this.config.sessions).some(session => session.isActive && this.isSessionActive(session, timestamp));
    }
    getNextSessionChange(timestamp = Date.now()) {
        let nextChange = null;
    ...
    updateCurrentSession(timestamp) {
    ...
    isSessionActive(session, timestamp) {
    ...
    getSessionTimes(session, timestamp) {
        return {
            start: session.startHour || 0,
            end: session.endHour || 0
        };
    }
    getHourUTC(timestamp) {
