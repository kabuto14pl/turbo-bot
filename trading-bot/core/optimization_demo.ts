...
class ObjectiveFunctions {
     ...
     */
    static rosenbrock(params: any): number {
        const x = params.x || 0;
        const y = params.y || 0;
     ...
     */
    static maCrossoverStrategy(params: any): number {
...
export class OptimizationDemo {
    ...
    constructor() {
        this.spaceManager = new HyperparameterSpaceManager();
        this.optimizationManager = new OptimizationManager(this.spaceManager);
    }
     ...
     */
    private async testSyntheticFunction(): Promise<void> {
        ...
        const objective: ObjectiveFunction = ObjectiveFunctions.sphere;
        
        const result = await this.optimizationManager.optimize(
            'genetic',
            spaceName,
            objective,
            100
        ...
        const x = (result.bestParameters as any).x || 0;
        const y = (result.bestParameters as any).y || 0;
     ...
     * Test strategii RSI
     */
    private async testRSIStrategy(): Promise<void> {
        console.log('📈 2. TEST STRATEGII RSI');
        console.log('   Cel: Optymalizacja parametrów RSI dla maksymalnego Sharpe ratio\n');

        const objective: ObjectiveFunction = ObjectiveFunctions.rsiStrategy;
     ...
     */
    private async testMACrossoverStrategy(): Promise<void> {
        console.log('📊 3. TEST STRATEGII MA CROSSOVER');
        console.log('   Cel: Optymalizacja parametrów MA dla najlepszego risk/reward\n');

        const objective: ObjectiveFunction = ObjectiveFunctions.maCrossoverStrategy;
     ...
     */
    private async compareAlgorithmsDemo(): Promise<void> {
        console.log('🏁 4. PORÓWNANIE ALGORYTMÓW');
        console.log('   Strategia: RSI z 100 iteracjami na algorytm\n');

        const objective: ObjectiveFunction = ObjectiveFunctions.rsiStrategy;
        
        const results = await this.optimizationManager.compareAlgorithms(
            'rsi_strategy',
            objective,
            100
        );
        ...
        const bestAlgorithm = Object.entries(results).reduce((best, [alg, result]) => 
     ...
     */
    private printResult(title: string, result: OptimizationResult): void {
        console.log(`✅ ${title}:`);
     ...
     */
    private printRSIParameters(result: OptimizationResult): void {
        const params = result.bestParameters as any;
     ...
     */
    private printMAParameters(result: OptimizationResult): void {
        const params = result.bestParameters as any;
     ...
     */
    private calculateImprovement(history: number[]): string {
