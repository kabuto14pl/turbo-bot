...
exports.ParallelOptimizationDemo = void 0;
/**
 * 🧪 [TESTING-FRAMEWORK]
...
class ParallelOptimizationDemo {
    constructor() {
        ...
        this.optimizationManager = new parallel_optimization_1.ParallelOptimizationManager({
            maxWorkers: 4,
            autoScale: true,
            resourceLimits: {
                maxCpuPercent: 80,
     ...
     */
    async demoMathematicalOptimization() {
        ...
        for (const [funcName, func] of Object.entries(objectiveFunctions)) {
            ...
            const results = await this.optimizationManager.runParallelOptimization(func, mathSpace, {
                algorithm: 'genetic',
                iterations: 1000,
                parallelRuns: 4,
                timeout: 30000
     ...
     */
    async demoTradingStrategyOptimization() {
        console.log('\n📈 Demo 2: Parallel Trading Strategy Optimization');
        console.log('='.repeat(60));
        ...
        for (const [strategyName, strategy] of Object.entries(tradingStrategies)) {
            ...
            const algorithmPromises = algorithms.map(async (algorithm) => {
                ...
                const results = await this.optimizationManager.runParallelOptimization(strategy, space, {
                    algorithm: algorithm,
                    iterations: 500,
                    parallelRuns: 2,
                    timeout: 45000
            ...
            const overallBest = algorithmResults.reduce((best, current) => current.bestScore > best.bestScore ? current : best);
     ...
     */
    async demoResourceScalingAndMonitoring() {
        console.log('\n🖥️  Demo 3: Resource Scaling and Monitoring');
        console.log('='.repeat(60));
        // Create a worker pool with detailed monitoring
        const workerPool = new parallel_optimization_1.RayLikeWorkerPool({
            maxWorkers: 6,
            autoScale: true,
            resourceLimits: {
                maxCpuPercent: 75,
    ...
    async demoPerformanceBenchmarking() {
        ...
        const benchmarkFunction = (params) => {
        ...
        for (const config of benchmarkConfigs) {
            ...
            const manager = new parallel_optimization_1.ParallelOptimizationManager({
                maxWorkers: config.workers,
                autoScale: false // Disable auto-scaling for consistent benchmarking
            });
            const startTime = Date.now();
            const optimizationResults = await manager.runParallelOptimization(benchmarkFunction, benchmarkSpace, {
                algorithm: 'genetic',
                iterations: 1000,
...
exports.ParallelOptimizationDemo = ParallelOptimizationDemo;
