...
exports.DynamicRebalancer = void 0;
exports.createProductionRebalancer = createProductionRebalancer;
exports.createConservativeRebalancer = createConservativeRebalancer;
class DynamicRebalancer {
     ...
     */
    async rebalanceWeights(currentTimestamp) {
     ...
     */
    async updatePerformanceMetrics(currentTimestamp) {
     ...
     */
    calculateOptimalWeights(currentTimestamp) {
    ...
    calculateStrategyScore(performance) {
        ...
        // Weighted score
        const score = (sharpeWeight * normalizedSharpe +
     ...
     */
    applyMomentumAdjustment(scores) {
        for (const [strategyId, performance] of this.strategyPerformances) {
            if (performance.returns.length < 7)
                continue; // Need at least 1 week
     ...
     */
    applyVolatilityAdjustment(scores) {
     ...
     */
    applyRebalanceConstraints(newWeights) {
     ...
     */
    filterOutliers(returns) {
        if (returns.length < 10)
            return returns; // Not enough data for outlier detection
    ...
    recordRebalanceHistory(timestamp, weights) {
     ...
     */
    updateStrategyPerformance(strategyId, return_, timestamp) {
...
exports.DynamicRebalancer = DynamicRebalancer;
/**
 * FACTORY FUNCTIONS
 */
function createProductionRebalancer(logger) {
    const config = {
...
function createConservativeRebalancer(logger) {
    const config = {
