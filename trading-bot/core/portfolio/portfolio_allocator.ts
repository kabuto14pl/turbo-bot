import { RollingMetric } from './portfolio_analytics';

export type AllocatorMode = 'minimum_variance' | 'risk_parity';

export interface AllocatorInput {
  returns: Record<string, RollingMetric[]>; // {strategy: [{timestamp, value}, ...]}
  volatility: Record<string, RollingMetric[]>; // rolling stddev
  correlations: Record<string, Record<string, RollingMetric[]>>; // correlations[a][b] = [{timestamp, value}, ...]
  window: number;
  timestamps: number[]; // zsynchronizowane timestampy
}

export interface Allocation {
  timestamp: number;
  weights: Record<string, number>; // {strategy: waga}
}

export class PortfolioAllocator {
  static allocate(input: AllocatorInput, mode: AllocatorMode = 'minimum_variance'): Allocation[] {
    ...
    for (const ts of timestamps) {
      ...
      const corrMatrix = strategies.map(a =>
        strategies.map(b =>
          a === b ? 1 : PortfolioAllocator._getMetricAt(correlations[a]?.[b] || [], ts) ?? 0
        )
      );
      // Rolling macierz kowariancji
      const covMatrix = corrMatrix.map((row, i) =>
        row.map((corr, j) => corr * vols[i] * vols[j])
      );
      let weights: number[] = [];
  ...
  static _minimumVarianceWeights(cov: number[][]): number[] {
  ...
  static _riskParityWeights(cov: number[][], tol = 1e-6, maxIter = 100): number[] {
    ...
    for (let iter = 0; iter < maxIter; iter++) {
      ...
      if (diff.every(d => Math.abs(d) < tol)) break;
  ...
  static _invertMatrix(mat: number[][]): number[][] {
  ...
  static _matVecMul(mat: number[][], vec: number[]): number[] {
    return mat.map(row => row.reduce((sum, v, i) => sum + v * vec[i], 0));
  }

  static _dot(a: number[], b: number[]): number {
    return a.reduce((sum, v, i) => sum + v * b[i], 0);
  }

  static _quadForm(w: number[], cov: number[][]): number {
    // w' * C * w
    return w.reduce((sum, wi, i) =>
      sum + wi * cov[i].reduce((s, cij, j) => s + cij * w[j], 0), 0);
  }

  static _marginalRiskContributions(w: number[], cov: number[][]): number[] {
    // C * w
    return cov.map(row => PortfolioAllocator._dot(row, w));
  }

  static _getMetricAt(arr: RollingMetric[], ts: number): number | undefined {
