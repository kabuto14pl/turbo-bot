...
export class AdvancedPortfolioManager extends EventEmitter {
    ...
    private eventHistory: PortfolioEvent[] = [];

    constructor(config: PortfolioConfig) {
        super();
        this.config = config;
        this.logger = new Logger('AdvancedPortfolioManager');
     ...
     */
    async processSignal(signal: Signal): Promise<Order[]> {
        try {
     ...
     */
    async rebalancePortfolio(): Promise<RebalancingAction[]> {
        try {
     ...
     */
    private async calculateTargetWeights(): Promise<Record<string, number>> {
     ...
     */
    private calculateEqualWeights(): Record<string, number> {
     ...
     */
    private calculateMarketCapWeights(): Record<string, number> {
        const weights: Record<string, number> = {};
        const totalMarketCap = this.config.assets.reduce((sum, asset) => 
            sum + (asset.marketCap || 1), 0);
     ...
     */
    private calculateRiskParityWeights(): Record<string, number> {
        const weights: Record<string, number> = {};
        const totalInverseVol = this.config.assets.reduce((sum, asset) => 
            sum + (1 / asset.volatility), 0);
     ...
     */
    private calculateMomentumWeights(): Record<string, number> {
        const weights: Record<string, number> = {};
     ...
     */
    private calculateMeanReversionWeights(): Record<string, number> {
        const weights: Record<string, number> = {};
        
        // Calculate mean reversion scores (inverse of recent performance)
        const reversionScores: Record<string, number> = {};
     ...
     */
    private async calculatePositionSize(asset: Asset, signal: Signal): Promise<number> {
        ...
        const baseSize = Math.min(
            maxPositionSize,
            (signal.strength * targetVolatility) / assetVolatility
        );
     ...
     * 🔍 Detect market regime changes
     */
    private async detectMarketRegime(marketData: MarketData[]): Promise<void> {
        // Simple regime detection based on volatility and trend
        const recentData = marketData.slice(-20); // Last 20 periods
        
        if (recentData.length < 10) return;

        const returns = recentData.slice(1).map((data, i) => 
            (data.close - recentData[i].close) / recentData[i].close
        );
    ...
    private calculatePortfolioVolatility(positions: PortfolioPosition[]): number {
        ...
        const weightedVol = positions.reduce((sum, pos) => 
            sum + (pos.weight * pos.performanceMetrics.volatility), 0);
        return weightedVol;
    }

    private calculateVaR(positions: PortfolioPosition[], confidence: number): number {
    ...
    private calculateCVaR(positions: PortfolioPosition[], confidence: number): number {
        const var95 = this.calculateVaR(positions, confidence);
        return var95 * 1.3; // Simplified CVaR
    }

    private calculateSharpeRatio(return_: number, volatility: number): number {
    ...
    private calculateSortino(positions: PortfolioPosition[]): number { return 0; }
    private calculateMaxDrawdown(positions: PortfolioPosition[]): number { return 0; }
    private calculateBeta(positions: PortfolioPosition[]): number { return 1; }
    private calculateAlpha(positions: PortfolioPosition[]): number { return 0; }
    private calculateDiversificationRatio(positions: PortfolioPosition[]): number { return 1; }
    private calculateCorrelationMatrix(): Record<string, Record<string, number>> { return {}; }
    private calculateRiskBudget(positions: PortfolioPosition[]): Record<string, number> { return {}; }
    private calculateRebalancingRisk(actions: RebalancingAction[]): number { return 0; }

    private createRebalancingAction(position: PortfolioPosition, targetWeight: number): RebalancingAction {
        const currentWeight = position.weight;
    ...
    private updatePositionPrices(marketData: MarketData[]): Promise<void> {
    ...
    // Public getters
    public getMetrics(): PortfolioMetrics { return { ...this.metrics }; }
    public getPositions(): PortfolioPosition[] { return Array.from(this.positions.values()); }
    public getCurrentRegime(): MarketRegime | null { return this.currentRegime; }
    public getEventHistory(): PortfolioEvent[] { return [...this.eventHistory]; }
}
