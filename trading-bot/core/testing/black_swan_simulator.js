...
// =====================================================
class BlackSwanSimulator extends events_1.EventEmitter {
    constructor() {
        super();
        this.testResults = [];
     ...
     */
    async simulateBlackSwanScenario(scenario) {
     ...
     */
    generateExtremeMarketConditions(scenario) {
     ...
     */
    simulatePortfolioUnderStress(extremeData, scenario) {
     ...
     */
    assessStrategyBreakdown(scenario) {
     ...
     */
    identifyCriticalFailures(portfolioImpact, scenario) {
        const failures = [];
     ...
     */
    identifySystemFailures(scenario) {
        const failures = [];
     ...
     */
    calculateVarExceedance(actualLoss) {
     ...
     */
    calculateStressedVolatility(scenario) {
     ...
     */
    analyzeRecoveryPotential(portfolioImpact, scenario) {
     ...
     */
    generateScenarioRecommendations(portfolioImpact, scenario) {
     ...
     */
    generateStressTestReport(results, testDuration) {
        ...
        let systemResilience;
        if (survivabilityRate < 25)
            systemResilience = 'FRAGILE';
        else if (survivabilityRate < 50)
            systemResilience = 'MODERATE';
        else if (survivabilityRate < 75)
            systemResilience = 'ROBUST';
        else
            systemResilience = 'ANTIFRAGILE';
        // Find worst and best case scenarios
        const worstCase = results.reduce((worst, current) => current.maxPortfolioLoss > worst.maxPortfolioLoss ? current : worst);
        const bestCase = results.reduce((best, current) => current.maxPortfolioLoss < best.maxPortfolioLoss ? current : best);
     ...
     */
    identifyLiquidityRisks(results) {
        const risks = [];
...
exports.BlackSwanSimulator = BlackSwanSimulator;
exports.default = BlackSwanSimulator;
