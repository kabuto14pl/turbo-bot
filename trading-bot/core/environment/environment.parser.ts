...
export type ExecutionMode = 'backtest' | 'demo' | 'production';
...
export class EnvironmentModeParser {
  private static instance: EnvironmentModeParser;
  private currentContext: EnvironmentContext | null = null;

  private constructor() {}

  public static getInstance(): EnvironmentModeParser {
   ...
   */
  public parseCommandLineArgs(): EnvironmentContext {
    ...
    if (modeIndex !== -1) {
      if (args[modeIndex].includes('=')) {
        mode = args[modeIndex].split('=')[1] as ExecutionMode;
      } else if (args[modeIndex + 1]) {
        mode = args[modeIndex + 1] as ExecutionMode;
      }
    }
    ...
    if (envIndex !== -1) {
      if (args[envIndex].includes('=')) {
        environment = args[envIndex].split('=')[1] as EnvironmentType;
      } else if (args[envIndex + 1]) {
        environment = args[envIndex + 1] as EnvironmentType;
      }
    }
   ...
   */
  public validateEnvironmentSafety(context: EnvironmentContext): string[] {
    const errors: string[] = [];
   ...
   */
  public setTestContext(context: EnvironmentContext): void {
...
`;
   ...
   */
  public getRecommendedExecutor(): 'SimulatedExecutor' | 'OKXExecutorAdapter' {
    if (!this.currentContext) return 'SimulatedExecutor';
...
export const environmentParser = EnvironmentModeParser.getInstance();

/**
 * Utility functions for main.ts integration
 */
export function parseEnvironmentAndValidate(): EnvironmentContext {
  ...
  console.log(environmentParser.generateEnvironmentReport());
  return context;
}

export function validateProductionDeployment(): boolean {
  const context = environmentParser.getCurrentContext();
  if (!context) return false;
  
  if (context.mode !== 'production') return true; // Not production, no validation needed
