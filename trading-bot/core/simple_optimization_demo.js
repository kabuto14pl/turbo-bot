...
async function main() {
    ...
    const spaceManager = new hyperparameter_space_1.HyperparameterSpaceManager();
    const optimizationManager = new simple_optimization_1.SimpleOptimizationManager(spaceManager);
    ...
    const sphereResult = await optimizationManager.optimize('genetic', 'sphere_test', TestObjectives.sphere, 100);
    ...
    const rsiResult = await optimizationManager.optimize('annealing', 'rsi_strategy', TestObjectives.rsiStrategy, 150);
    ...
    const maResult = await optimizationManager.optimize('random', 'ma_crossover', TestObjectives.maCrossover, 120);
    ...
    const comparison = await optimizationManager.compareAlgorithms('rsi_strategy', TestObjectives.rsiStrategy, 80);
    ...
    const bestAlgorithm = Object.entries(comparison).reduce((best, [alg, result]) => result.bestScore > best[1].bestScore ? [alg, result] : best);
