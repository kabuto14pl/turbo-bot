...
import { AutoHedgingEngine, HedgeType, HedgeTrigger } from './auto_hedging_engine';

// =====================================================
// DELTA-NEUTRAL INTERFACES & TYPES
// =====================================================

export interface DeltaMetrics {
    portfolioDelta: number;
    assetDeltas: Map<string, number>;
    totalExposure: number;
    neutralityScore: number; // 0-1, where 1 is perfectly neutral
    lastCalculated: number;
    requiredAdjustments: DeltaAdjustment[];
}
...
export interface DeltaNeutralConfig {
    enabled: boolean;
    targetDelta: number; // Target portfolio delta (usually 0)
    deltaThreshold: number; // Rebalance when |delta| exceeds this
    rebalanceFrequency: number; // minutes
    maxAdjustmentSize: number; // Max position size adjustment per rebalance
    neutralityTarget: number; // Target neutrality score (0.9-1.0)
    hedgingPreference: 'POSITIONS' | 'DERIVATIVES' | 'MIXED';
    autoRebalance: boolean;
    emergencyRebalance: boolean;
    riskLimits: {
        maxPortfolioDelta: number;
        maxAssetDelta: number;
...
export interface RebalanceResult {
    ...
    totalCost: number;
    neutralityImprovement: number;
    errorMessage?: string;
    executionTime: number;
    details: RebalanceDetail[];
}
...
export class DeltaNeutralManager extends EventEmitter {
    ...
    constructor(
        ...
        config?: Partial<DeltaNeutralConfig>
    ) {
        super();
        this.logger = logger;
     ...
     */
    async calculatePortfolioDelta(): Promise<DeltaMetrics> {
        try {
            ...
            const requiredAdjustments = await this.generateDeltaAdjustments(
                normalizedPortfolioDelta,
                assetDeltas,
                totalExposure
            );
     ...
     */
    private async calculatePositionDelta(position: Position): Promise<number> {
        try {
     ...
     */
    private async generateDeltaAdjustments(
        portfolioDelta: number,
        assetDeltas: Map<string, number>,
        totalExposure: number
    ): Promise<DeltaAdjustment[]> {
        ...
        for (const [symbol, currentDelta] of sortedAssets) {
            if (Math.abs(remainingDeltaToAdjust) < 0.01) break;

            const position = this.portfolio.getPosition(symbol);
            if (!position) continue;

            // Calculate optimal adjustment for this asset
            const adjustmentSize = Math.min(
                Math.abs(remainingDeltaToAdjust) * 0.5, // Take 50% of remaining adjustment
            ...
            if (adjustmentSize > 0.01) { // Only create adjustment if significant
                ...
                const adjustment: DeltaAdjustment = {
     ...
     */
    async maintainDeltaNeutrality(): Promise<RebalanceResult> {
     ...
     */
    private async executeRebalance(adjustments: DeltaAdjustment[]): Promise<{
        success: boolean;
        adjustmentsMade: number;
        totalCost: number;
        details: RebalanceDetail[];
     ...
     */
    private async executeAdjustment(adjustment: DeltaAdjustment): Promise<RebalanceDetail> {
     ...
     */
    private async executePositionAdjustment(adjustment: DeltaAdjustment): Promise<{
        success: boolean;
        cost: number;
        deltaChange: number;
    }> {
     ...
     */
    private async executeHedgeAdjustment(adjustment: DeltaAdjustment): Promise<{
        success: boolean;
        cost: number;
        deltaChange: number;
    }> {
        try {
            ...
            const hedgeTrigger: HedgeTrigger = {
     ...
     */
    private addToHistory(metrics: DeltaMetrics): void {
     ...
     */
    private determineAdjustmentAction(currentDelta: number, targetDelta: number): 'BUY' | 'SELL' | 'HEDGE' {
     ...
     */
    private calculateAdjustmentPriority(portfolioDelta: number): 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL' {
     ...
     */
    private estimateAdjustmentCost(symbol: string, adjustmentSize: number): number {
     ...
     */
    private calculateAdjustmentConfidence(symbol: string, adjustmentSize: number): number {
     ...
     */
    getCurrentMetrics(): DeltaMetrics | null {
     ...
     */
    async forceRebalance(): Promise<RebalanceResult> {
...
export default DeltaNeutralManager;
