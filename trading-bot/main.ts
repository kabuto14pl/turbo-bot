...
import { configManager, ConfigurationProfile } from './config/environments/config.manager';
import { EnvironmentConfig } from './config/environments/base.config';

import { loadCandles } from './infrastructure/data/csv_loader';
import { joinCandlesByTimestamp } from './core/utils/MultiTFJoiner';
import { BotState } from './core/types/bot_state';
import { buildReport } from './analytics/reportBuilder';
import * as path from 'path';
import { aggregateCandles } from './tools/aggregate_csv';
import * as fs from 'fs';
import { generateNavChart } from './analytics/visual_report_builder';
import { Candle, StrategySignal } from './core/types/strategy';
import { EnhancedStrategySignal } from './core/types/enhanced_strategy_signal';
import { Portfolio } from './core/portfolio/index';
import { PerformanceTracker } from './core/analysis/performance_tracker';
import { CooldownManager } from './core/risk/cooldown_manager';
...
import { generateEquityCurve } from './analytics/stats';
import { createProductionRegimeFilter, RegimeFilter } from './core/analysis/regime_filter';
...
import { AbstractStrategy } from './core/strategy/abstract_strategy';
...
import { DataPreparationService, DataSourceConfig } from './core/services/data_preparation_service';
import { IndicatorProvider } from './core/indicators/IndicatorProvider';
import {
  PortfolioAllocator,
  AllocatorInput,
  AllocatorMode,
} from './core/portfolio/portfolio_allocator';
...
import { AutoHedgingSystemFactory, AutoHedgingSystem, AutoHedgingSystemConfig, HedgeExecutionAdapter } from './core/hedging';
...
// import { DashboardIntegration } from './dashboard/dashboard_integration';

// --- NOWE TYPY KONFIGURACJI ---
...
interface StrategyConfig {
  name: StrategyName;
  params: any; // np. { rsiPeriod: 14 } lub { stMultiplier: 3 }
}
...
const signal_events: any[] = [];
const trade_events: any[] = [];
function record_signal(strategy: string, ts: number, type: string, price: number, extra: any = {}) {
...
async function initializeSystem() {
  try {
    ...
    const configProfile: ConfigurationProfile = `${mode}.default` as ConfigurationProfile;
    const config = configManager.loadConfiguration(configProfile);
...
function determineTrend(prices: number[]): 'bullish' | 'bearish' | 'sideways' {
...
export async function runTest(config: TestConfig, candles15m: any[], kafkaEngine?: any) {
  ...
  const rollingPrices: Record<string, Record<string, number>> = {};
  // --- DODANE: Deklaracja rollingVaR ---
  let rollingVaR: { timestamp: number; value: number }[] = [];
  ...
  const strategyEquity: Record<string, { timestamp: number; nav: number }[]> = {};
  ...
  if (config.autoHedging?.enabled) {
    ...
    try {
      autoHedgingSystem = AutoHedgingSystemFactory.create(
        new Logger(),
        globalPortfolio as any, // Temporary type cast to resolve Portfolio compatibility
  ...
  if (config.executionMode === 'live' || config.executionMode === 'demo') {
    ...
    tradeExecutor = new SimulatedExecutor(
      new Logger(),
      globalPortfolio,
      new RiskManager(new Logger()),
  ...
  if (autoHedgingSystem && tradeExecutor) {
    try {
      ...
      const hedgeExecutionAdapter = new HedgeExecutionAdapter(
        new Logger(),
        tradeExecutor
      );
  ...
  const regimeFilter = createProductionRegimeFilter(new Logger());

  const cooldownManagers: Record<string, CooldownManager> = {};
  config.strategies.forEach(s => {
    cooldownManagers[s.name] = new CooldownManager();
  });

  // --- DODANE: Instancja DuckDBAdapter ---
  const duckdb = new DuckDBAdapter('trading_data.duckdb');

  // --- 2. PRZYGOTOWANIE DANYCH RYNKOWYCH ---
  const dataSourceConfig: DataSourceConfig = { type: 'csv', path: './data/BTCUSDT/' };
  const dataPreparationService = new DataPreparationService(dataSourceConfig);
  ...
  for (const market of preparedMarkets) {
    if (
      ...
      console.warn(
        `[DIAGNOSTYKA] ${market.symbol}: braki=${market.diagnostics.missingBars}, duplikaty=${market.diagnostics.duplicateTimestamps}`
      );
    }
  }
  ...
  const local_signal_events: any[] = [];
  const local_trade_events: any[] = [];
  ...
  for (const market of allSingleMarkets) {
    ...
    const strategies: { name: string; strat: any }[] = config.strategies.map(
      s_config => {
        ...
        switch (s_config.name) {
          ...
          case 'EnhancedRSITurbo':
            return { name: s_config.name, strat: new EnhancedRSITurboStrategy(logger) };
          case 'SuperTrend':
            return { name: s_config.name, strat: new SuperTrendStrategy(logger) };
          case 'MACrossover':
            return { name: s_config.name, strat: new MACrossoverStrategy(logger) };
          case 'MomentumConfirm':
            return { name: s_config.name, strat: new MomentumConfirmationStrategy(logger) };
          case 'MomentumPro':
            return { name: s_config.name, strat: new MomentumProStrategy(logger) };
    ...
    const portfolioManager = new AdvancedPortfolioManager(portfolioConfig);
    const riskManager = new AdvancedRiskManager(portfolioConfig.riskLimits);
    const portfolioOptimizer = new AdvancedPortfolioOptimizer();
    ...
    for (let i = 0; i < joinedCandles.length; i++) {
      ...
      const m15Indicators: IndicatorSet = {
        rsi: calcRSI(m15Candles, 14) ?? 50,
        ema_9: calcEMA(m15Candles, 9) ?? ctx.base.close,
        ema_21: calcEMA(m15Candles, 21) ?? ctx.base.close,
        ema_50: calcEMA(m15Candles, 50) ?? ctx.base.close,
        ema_200: calcEMA(m15Candles, 200) ?? ctx.base.close,
        adx: calculateADX(
          m15Candles.map(c => c.high),
          m15Candles.map(c => c.low),
          m15Candles.map(c => c.close)
        )?.[m15Candles.length - 1] ?? 25,
        atr: calculateATR(
          m15Candles.map(c => c.high),
          m15Candles.map(c => c.low),
          m15Candles.map(c => c.close)
        )[m15Candles.length - 1] ?? ctx.base.close * 0.01,
        supertrend: { value: ctx.base.close, direction: 'buy' },
        macd: { macd: 0, signal: 0, histogram: 0 }
      };

      const h1Indicators: IndicatorSet = {
        rsi: calcRSI(h1Candles, 14) ?? 50,
        ema_9: calcEMA(h1Candles, 9) ?? ctx.base.close,
        ema_21: calcEMA(h1Candles, 21) ?? ctx.base.close,
        ema_50: calcEMA(h1Candles, 50) ?? ctx.base.close,
        ema_200: calcEMA(h1Candles, 200) ?? ctx.base.close,
        adx: calculateADX(
          h1Candles.map(c => c.high),
          h1Candles.map(c => c.low),
          h1Candles.map(c => c.close)
        )?.[h1Candles.length - 1] ?? 25,
        atr: calculateATR(
          h1Candles.map(c => c.high),
          h1Candles.map(c => c.low),
          h1Candles.map(c => c.close)
        )[h1Candles.length - 1] ?? ctx.base.close * 0.01,
        supertrend: { value: ctx.base.close, direction: 'buy' },
        macd: { macd: 0, signal: 0, histogram: 0 }
      };

      const h4Indicators: IndicatorSet = {
        rsi: calcRSI(h4Candles, 14) ?? 50,
        ema_9: calcEMA(h4Candles, 9) ?? ctx.base.close,
        ema_21: calcEMA(h4Candles, 21) ?? ctx.base.close,
        ema_50: calcEMA(h4Candles, 50) ?? ctx.base.close,
        ema_200: calcEMA(h4Candles, 200) ?? ctx.base.close,
        adx: calculateADX(
          h4Candles.map(c => c.high),
          h4Candles.map(c => c.low),
          h4Candles.map(c => c.close)
        )?.[h4Candles.length - 1] ?? 25,
        atr: calculateATR(
          h4Candles.map(c => c.high),
          h4Candles.map(c => c.low),
          h4Candles.map(c => c.close)
        )[h4Candles.length - 1] ?? ctx.base.close * 0.01,
        supertrend: { value: ctx.base.close, direction: 'buy' },
        macd: { macd: 0, signal: 0, histogram: 0 }
      };

      const d1Indicators: IndicatorSet = {
        rsi: 50,
        ...
        adx: 25,
        atr: calculateATR(
          d1Candles.map(c => c.high),
          d1Candles.map(c => c.low),
          d1Candles.map(c => c.close)
        )[d1Candles.length - 1] ?? ctx.base.close * 0.01,
        supertrend: { value: ctx.base.close, direction: 'buy' },
        macd: { macd: 0, signal: 0, histogram: 0 }
      };
      ...
      if (ctx && ctx.base && ctx.base.time && ctx.base.close) {
        if (!rollingPrices[ctx.base.time]) rollingPrices[ctx.base.time] = {};
      ...
      if (i < 5 && botState.indicators) {
        console.log(
          `[DEBUG] Bar ${i}: RSI=${botState.indicators.m15.rsi}, EMA200=${botState.indicators.m15.ema_200}, ADX=${botState.indicators.m15.adx}, ATR=${botState.indicators.m15.atr}`
        );
      }
      ...
      for (const { name, strat } of strategies) {
        ...
        const currentVaR = rollingVaR.find(
          (v: { timestamp: number; value: number }) => v.timestamp === ctx.base.time
        )?.value;
        const riskManager =
          tradeExecutor.getRiskManager && typeof tradeExecutor.getRiskManager === 'function'
        ...
        if (!strategyEquity[name]) strategyEquity[name] = [];
        ...
        prometheusMonitoring.updatePortfolioMetrics(
          portfolioValue,
          { [symbol]: portfolioValue }, // Simple exposure tracking
        ...
        const logger = new Logger();
        const strategy = new EnhancedRSITurboStrategy(logger);
        
        // 🚀 SENTIMENT-ENHANCED SIGNAL GENERATION
        let signals: StrategySignal[] = [];
        let enhancedSignal: EnhancedStrategySignal | null = null;
        ...
        if (signals && signals.length > 0) {
          for (const signal of signals) {
            ...
            for (const order of executedOrders) {
              ...
              console.log(
                `[EXECUTION] Zrealizowano zlecenie ${order.type} (${action}) dla ${name} w cenie ${order.executedPrice}. PnL: ${order.pnl}`
              );
              ...
              if (order.pnl !== null && order.pnl !== undefined) {
                ...
                try {
                  ...
                  await enterpriseMLManager.learnFromResult(
                    order.pnl,
                    trade_duration,
                    {
            ...
            const orderRequest: OrderRequest | null = signal.type.includes('ENTER')
              ? {
                  symbol,
                  type: 'market',
            ...
            if (orderRequest) {
              ...
              if (!globalRiskManager.canOpenPosition()) {
                console.log(
                  `[GLOBAL RISK] ${name} ts=${ctx.base.time} Pomijam open (${orderRequest.side}), bo przekroczono globalny limit ryzyka.`
                );
                continue;
              }
      ...
      try {
        const rlAction = await enterpriseMLManager.processStep(
          ctx.base.close,
          botState.indicators.m15.rsi,
          ctx.base.volume
        );
  ...
  function exportAllocCSV(
    filename: string,
    alloc: { timestamp: number; weights: Record<string, number> }[]
  ) {
  ...
  exportAllocCSV(
    'rolling_alloc_minvar.csv',
    PortfolioAllocator.allocate(
      {
        returns: Object.fromEntries(
          Object.entries(strategyEquity).map(([k, v]) => [
            k,
            v.map((x, i, arr) =>
              i === 0
  ...
  const tradesWithPnl = trades.filter(
    t => t.event === 'close' && typeof t.pnl === 'number'
  ) as any[];
  ...
  // --- Portfolio Analytics ---
  const portfolioAnalytics = new PortfolioAnalytics(globalPortfolio);
  const portfolioStats = portfolioAnalytics.getStats();
  fs.writeFileSync(
    path.join(testDirectory, 'portfolio_stats.json'),
  ...
  function exportRollingMetricCSV(
    filename: string,
  ...
  // --- Raport statystyczny ---
  buildReport(tradesWithPnl, testDirectory);

  // --- Eksport CSV transakcji ---
  const csvHeader = 'timestamp,symbol,side,price,size,pnl,margin,strategy\n';
  const csvRows = trades.map((t: any) =>
    [t.timestamp, t.symbol, t.direction, t.price, t.size, t.pnl, t.margin, t.strategy].join(',')
  );
  fs.writeFileSync(
    path.join(testDirectory, `trades_global.csv`),
  ...
  const pricesRows: { timestamp: number; [symbol: string]: number }[] = Object.keys(
    rollingPrices
  ).map((ts: string) => ({ timestamp: Number(ts), ...rollingPrices[ts] }));
  if (pricesRows.length > 0) {
    await duckdb.saveSeries(`rolling_prices_${config.id}`, pricesRows);
  }

  // --- 6. GENEROWANIE RAPORTU KOŃCOWEGO ---
  const stats = buildReport(local_trade_events, testDirectory);
  generateNavChart(globalPortfolio.getNavHistory(), testDirectory);
  ...
  if (sentimentReport.insights.length > 0) {
    console.log('\n💡 INSIGHTS:');
    sentimentReport.insights.forEach((insight: string) => console.log(`   ${insight}`));
  }

  if (sentimentReport.recommendations.length > 0) {
    console.log('\n🔧 RECOMMENDATIONS:');
    sentimentReport.recommendations.forEach((rec: string) => console.log(`   ${rec}`));
  }

  // Save sentiment report to file
  fs.writeFileSync(
    path.join(testDirectory, 'sentiment_performance_report.json'),
...
async function main() {
  ...
  const deploymentOrchestrator = new DeploymentOrchestrator(false); // PRODUCTION MODE
  ...
  const kafkaEngine = new KafkaRealTimeStreamingEngine({
    kafka: {
  ...
  deploymentApp.post('/api/deploy/blue-green', async (req: any, res: any) => {
    try {
  ...
  deploymentApp.post('/api/config/hot-reload', async (req: any, res: any) => {
    try {
  ...
  deploymentApp.post('/api/deploy/rolling-update', async (req: any, res: any) => {
    try {
  ...
  deploymentApp.post('/api/deploy/migrate', async (req: any, res: any) => {
    try {
  ...
  deploymentApp.post('/api/deploy/full', async (req: any, res: any) => {
    try {
  ...
  deploymentApp.get('/api/deploy/status', (req: any, res: any) => {
    try {
      const blueGreenManager = deploymentOrchestrator.getBlueGreenManager();
      const status = (blueGreenManager as any).getDeploymentStatus();
  ...
  try {
    ...
    if ((testResult as any)?.dashboardIntegration) {
      console.log('📊 Zatrzymywanie Dashboard Integration...');
      await (testResult as any).dashboardIntegration.stop();
