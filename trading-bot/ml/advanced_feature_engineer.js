...
exports.AdvancedFeatureEngineer = void 0;
const events_1 = require("events");
const logger_1 = require("../infrastructure/logging/logger");
class AdvancedFeatureEngineer extends events_1.EventEmitter {
    constructor() {
        super();
        ...
        this.logger = new logger_1.Logger('AdvancedFeatureEngineer');
     ...
     */
    async extractFeatures(candles, symbol, timeframe) {
     ...
     */
    async extractTechnicalFeatures(candles) {
        const features = {};
     ...
     */
    async extractStatisticalFeatures(candles) {
        const features = {};
        const prices = candles.map(c => c.close);
     ...
     */
    async extractTemporalFeatures(candle) {
        const features = {};
     ...
     */
    async extractMarketStructureFeatures(candles) {
        const features = {};
        const prices = candles.map(c => c.close);
        const highs = candles.map(c => c.high);
        const lows = candles.map(c => c.low);
    ...
    calculateRSIDivergence(prices, period) {
    ...
    calculatePercentileRank(values, target) {
    ...
    calculateTrendDirection(prices, period) {
        if (prices.length < period)
            return 0;
        const recentPrices = prices.slice(-period);
    ...
    detectMarketRegime(prices, period) {
        if (prices.length < period)
            return 0;
        const recentPrices = prices.slice(-period);
    ...
    calculateVolumeImbalance(volumes, period) {
    ...
    detectDoubleBottom(lows, period) {
    ...
    detectTrianglePattern(highs, lows, period) {
    ...
    calculateFibonacciLevels(highs, lows, period) {
        const recentHighs = highs.slice(-period);
        const recentLows = lows.slice(-period);
...
exports.AdvancedFeatureEngineer = AdvancedFeatureEngineer;
exports.default = AdvancedFeatureEngineer;
