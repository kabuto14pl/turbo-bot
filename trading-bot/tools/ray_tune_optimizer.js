...
exports.RayTuneOptimizer = void 0;
/**
...
class RayTuneOptimizer {
     ...
     */
    async optimize(objectiveFunction, parameterSpace) {
     ...
     */
    async runOptimization(objectiveFunction, parameterSpace) {
        ...
        const handlePythonProcess = () => {
            return new Promise((resolve, reject) => {
                ...
                const rayProcess = (0, child_process_1.spawn)(PYTHON_VENV_PATH, [
                    wslPythonScript,
        ...
        return {
            ...
            allTrials: rawResults.all_trials.map((trial) => ({
     ...
     */
    async optimizeWithPython(objectiveFunctionPath, parameterSpace, additionalArgs = {}) {
        // Utwórz plik konfiguracyjny dla Pythona
        const fullConfig = {
            experiment_id: this.experimentId,
            parameter_space: parameterSpace,
     ...
     */
    async readRayTrials(experimentPath) {
        const trials = [];
     ...
     */
    processTrialsToResult(trials) {
        const completedTrials = trials.filter(trial => trial.status === 'TERMINATED' && trial.metrics && trial.metrics.score !== undefined);
    ...
    processResults(rawResults) {
        const completedTrials = rawResults.trials.filter((trial) => trial.status === 'TERMINATED' && trial.metrics && trial.metrics.score !== undefined);
        if (completedTrials.length > 0) {
            const bestTrial = completedTrials.reduce((best, current) => {
                const currentScore = current.metrics.score;
                const bestScore = best.metrics.score;
            ...
            return {
                ...
                allTrials: rawResults.trials.map((trial) => ({
                    parameters: trial.config,
                    score: trial.metrics?.score,
                    status: trial.status
                })),
                trials: rawResults.trials.map((trial) => ({
                    params: trial.config,
                    score: trial.metrics?.score,
                    status: trial.status
                })),
...
exports.RayTuneOptimizer = RayTuneOptimizer;
