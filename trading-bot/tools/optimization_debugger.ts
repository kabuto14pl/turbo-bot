...
/**
 * Konfiguracja opcji debugowania
 */
interface DebugOptions {
    logToConsole: boolean;     // Czy wyświetlać logi w konsoli
    logToFile: boolean;        // Czy zapisywać logi do pliku
    logDirectory: string;      // Katalog, w którym będą zapisywane logi
    logLevel: 'basic' | 'detailed' | 'verbose';  // Poziom szczegółowości logów
    saveIntermediateResults: boolean;  // Czy zapisywać wyniki pośrednie
    parameterSteps: boolean;   // Czy logować każdy krok zmiany parametrów
    visualizeProgress: boolean; // Czy generować wizualizacje postępu
}

/**
 * Klasa do debugowania procesu optymalizacji
 */
export class OptimizationDebugger {
    private options: DebugOptions;
    private logFilePath: string;
    private startTime: number;
    private iterationResults: Record<string, any[]> = {};
    private currentStrategy: string = '';
    private currentIteration: number = 0;
    private totalIterations: number = 0;
    private bestResults: Record<string, any> = {};
     ...
     */
    log(message: string, level: 'basic' | 'detailed' | 'verbose' = 'basic'): void {
     ...
     */
    startStrategyOptimization(strategyName: string, config: TestConfig, trials: number): void {
        ...
        if (this.options.saveIntermediateResults) {
            ...
            fs.writeFileSync(
                path.join(configDir, `${strategyName}_config.json`),
    ...
    logIteration(iteration: number, params: any, metrics: any): void {
        ...
        const metricsStr = Object.entries(metrics)
            .map(([key, value]) => `${key}: ${typeof value === 'number' ? (value as number).toFixed(4) : value}`)
            .join(', ');
     ...
     */
    private updateBestResult(params: any, metrics: any): void {
     ...
     */
    finishStrategyOptimization(finalResult: any): void {
        const elapsedTime = this.formatTime(Date.now() - this.startTime);
        ...
        if (this.options.saveIntermediateResults) {
            ...
            fs.writeFileSync(
                path.join(resultsDir, `${this.currentStrategy}_all_iterations.json`),
                JSON.stringify(this.iterationResults[this.currentStrategy], null, 2),
                'utf8'
            );
            
            // Zapisz najlepszy wynik
            fs.writeFileSync(
                path.join(resultsDir, `${this.currentStrategy}_best_result.json`),
     ...
     */
    logError(error: any, iteration?: number, params?: any): void {
        ...
        if (this.options.logToFile) {
            ...
            const errorFilePath = path.join(
                errorsDir, 
                `error_${this.currentStrategy}_iteration_${iteration || 'unknown'}_${Date.now()}.log`
            );
    ...
    private visualizeProgress(): void {
        if (!this.options.visualizeProgress) return;
        ...
        fs.writeFileSync(
            path.join(visualsDir, `progress_${this.currentStrategy}_iteration_${this.currentIteration}.html`),
            htmlContent,
            'utf8'
        );
     ...
     */
    private generateFinalVisualization(): void {
        if (!this.options.visualizeProgress) return;
        ...
        fs.writeFileSync(
            path.join(visualsDir, `final_report_${this.currentStrategy}.html`),
            htmlContent,
            'utf8'
        );
     ...
     */
    generateSummaryReport(outputDir: string): void {
        ...
        const summaryData = {
            ...
            iterationCounts: Object.fromEntries(
                Object.entries(this.iterationResults).map(
                    ([strategy, results]) => [strategy, results.length]
                )
            )
        };
        
        fs.writeFileSync(
            path.join(reportDir, 'debug_summary.json'),
     ...
     */
    private generateHtmlReport(reportDir: string, summaryData: any): void {
...
export const optimizationDebugger = new OptimizationDebugger();
