...
function saveResults(results, strategyName, params) {
    ...
    // Zapisz wyniki
    fs.writeFileSync(path.join(dir, `${strategyName}_results.json`), JSON.stringify(results, null, 2));
    // Zapisz parametry
    fs.writeFileSync(path.join(dir, `${strategyName}_params.json`), JSON.stringify(params, null, 2));
    // Zapisz trades
    fs.writeFileSync(path.join(dir, `${strategyName}_trades.json`), JSON.stringify(results.trades, null, 2));
    // Zapisz sygnały
    fs.writeFileSync(path.join(dir, `${strategyName}_signals.json`), JSON.stringify(results.signals, null, 2));
...
async function main() {
    ...
    const candles = await (0, csv_loader_1.loadCandles)('BTC_data_15m.csv');
