...
const trades: Trade[] = JSON.parse(fs.readFileSync(tradesFile, 'utf-8'));
...
const pnlValues = trades.map((t: Trade) => t.pnl);
const pnlGroups = {
    '< -1000': pnlValues.filter((p: number) => p <= -1000).length,
    '-1000 to -500': pnlValues.filter((p: number) => p > -1000 && p <= -500).length,
    '-500 to 0': pnlValues.filter((p: number) => p > -500 && p <= 0).length,
    '0 to 500': pnlValues.filter((p: number) => p > 0 && p <= 500).length,
    '500 to 1000': pnlValues.filter((p: number) => p > 500 && p <= 1000).length,
    '> 1000': pnlValues.filter((p: number) => p > 1000).length
};
