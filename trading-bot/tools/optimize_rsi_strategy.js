        ...
        for (const oversold of oversoldLevels) {
            for (const overbought of overboughtLevels) {
                if (oversold >= overbought)
                    continue; // Ignoruj nieprawidłowe kombinacje
...
function cleanCandleData(candles) {
    ...
    if (filteredCandles.length < candles.length * 0.7) {
        ...
        for (let i = 1; i < fixedCandles.length; i++) {
            ...
            if (isNaN(currentCandle.open))
                currentCandle.open = prevCandle.close;
            if (isNaN(currentCandle.high))
                currentCandle.high = Math.max(prevCandle.high, currentCandle.open);
            if (isNaN(currentCandle.low))
                currentCandle.low = Math.min(prevCandle.low, currentCandle.open);
            if (isNaN(currentCandle.close))
                currentCandle.close = currentCandle.open;
            if (isNaN(currentCandle.volume))
                currentCandle.volume = prevCandle.volume;
        }
        // Teraz sprawdź czy nadal są jakieś NaN w pierwszej świecy i napraw je
        if (isNaN(fixedCandles[0].open))
            fixedCandles[0].open = 0;
        if (isNaN(fixedCandles[0].high))
            fixedCandles[0].high = 0;
        if (isNaN(fixedCandles[0].low))
            fixedCandles[0].low = 0;
        if (isNaN(fixedCandles[0].close))
            fixedCandles[0].close = 0;
        if (isNaN(fixedCandles[0].volume))
            fixedCandles[0].volume = 0;
...
async function runOptimizationForTimeframe(timeframeName, filePath) {
    ...
    const rawCandles = await (0, csv_loader_1.loadCandles)(filePath);
...
async function main() {
    try {
        ...
        for (const timeframeResult of allTimeframeResults) {
            ...
            fs.writeFileSync(path.join(timeframeDir, 'best_params.json'), JSON.stringify(timeframeResult.results.bestResult?.params, null, 2));
            // Zapisz statystyki najlepszego rezultatu
            fs.writeFileSync(path.join(timeframeDir, 'best_stats.json'), JSON.stringify(timeframeResult.results.bestResult?.stats, null, 2));
            // Zapisz top 10 wyników
            const top10 = timeframeResult.results.allResults.slice(0, 10);
            fs.writeFileSync(path.join(timeframeDir, 'top10_results.json'), JSON.stringify(top10.map(r => ({
                params: r.params,
                stats: r.stats
            })), null, 2));
        }
