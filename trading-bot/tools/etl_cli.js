...
function printResult(result) {
    ...
    if (result.validation) {
        ...
        if (result.validation.errors.length > 0) {
            ...
            result.validation.errors.slice(0, 5).forEach((error, index) => {
        ...
        if (result.validation.warnings.length > 0) {
            ...
            result.validation.warnings.slice(0, 5).forEach((warning, index) => {
...
async function main() {
    ...
    try {
        ...
        // Utwórz procesor ETL
        const processor = new etl_processor_1.ETLProcessor(config);
        // Przetwórz plik
