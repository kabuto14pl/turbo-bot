...
async function optimizeStrategy(strategyName, candles, maxCombinations = 100) {
    ...
    for (let i = 0; i < allCombinations.length; i++) {
        ...
        const testConfig = {
            ...
            strategies: [
                {
                    name: strategyName, // UWAGA: Wymuszamy typ
                    params
                }
            ],
            symbols: ['BTCUSDT'],
    ...
    if (bestParams && bestStats) {
        ...
        fs.writeFileSync(path.join(outputDir, 'best_params.json'), JSON.stringify({ strategy: strategyName, params: bestParams, stats: bestStats }, null, 2));
    }
    else {
...
async function runFullOptimization() {
    ...
    fs.writeFileSync(path.join(summaryDir, 'optimization_summary.json'), JSON.stringify(allResults, null, 2));
