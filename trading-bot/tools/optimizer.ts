...
// Typ dla konfiguracji optymalizacji
export interface OptimizationConfig {
    strategyName: StrategyName;
    paramRanges: ParamRange;
    baseConfig: Omit<TestConfig, 'strategies' | 'id'>;
    outputDir: string;
}

// Typ dla pojedynczego wyniku testu
interface TestResult {
    params: any;
    stats: any; // Typ statystyk zwrócony przez runTest
}

/**
 * Generuje wszystkie możliwe kombinacje parametrów.
 */
function generateParamCombinations(ranges: ParamRange): any[] {
 ...
 */
export async function runOptimization(config: OptimizationConfig, candles: any[]) {
    console.log('Starting Bayesian optimization (Optuna)...');
    await runOptunaBayesianOptimization();
 ...
 */
function generateOptimizationReport(results: TestResult[], outputDir: string) {
