...
/**
 * Interfejs definiujący metadane eksperymentu
 */
export interface ExperimentMetadata {
    id: string;                   // Unikalny identyfikator eksperymentu
    name: string;                 // Nazwa eksperymentu
    description?: string;         // Opcjonalny opis
    strategyName: string;         // Nazwa strategii
    createdAt: number;            // Timestamp utworzenia
    completedAt?: number;         // Timestamp zakończenia (jeśli zakończono)
    duration?: number;            // Czas trwania w milisekundach (jeśli zakończono)
    profile: string;              // Nazwa użytego profilu optymalizacji
    profileSettings: OptimizationProfile; // Pełne ustawienia profilu
    status: 'running' | 'completed' | 'failed' | 'interrupted'; // Status eksperymentu
    error?: string;               // Opis błędu (jeśli status = 'failed')
    metrics: {                    // Wyniki metryk (jeśli zakończono)
        [key in OptimizationMetric]?: number;
    };
    bestParams: Record<string, any>; // Najlepsze znalezione parametry
    tags: string[];               // Tagi do łatwego wyszukiwania
}

/**
 * Interfejs określający filtr do wyszukiwania eksperymentów
 */
export interface ExperimentFilter {
    strategyName?: string;        // Filtrowanie po strategii
    profileName?: string;         // Filtrowanie po profilu
    tags?: string[];              // Filtrowanie po tagach
    dateRange?: {                 // Filtrowanie po zakresie dat
        from?: number;
        to?: number;
    };
    metrics?: {                   // Filtrowanie po wartościach metryk
        [key in OptimizationMetric]?: {
            min?: number;
            max?: number;
        };
    };
    status?: ('running' | 'completed' | 'failed' | 'interrupted')[]; // Filtrowanie po statusie
}
...
export class ExperimentTracker {
    ...
    private experimentsCache: Record<string, ExperimentMetadata> = {};
     ...
     */
    updateExperiment(id: string, update: Partial<ExperimentMetadata>): void {
     ...
     */
    completeExperiment(id: string, results: {
     ...
     */
    logIteration(id: string, iteration: number, params: any, metrics: any): void {
        if (!this.experimentsCache[id]) {
            throw new Error(`Eksperyment o ID ${id} nie istnieje`);
        }
        ...
        fs.writeFileSync(
            path.join(iterationsDir, `iteration_${iteration}.json`),
            JSON.stringify({ iteration, params, metrics }, null, 2)
        );
    }
     ...
     */
    getExperiment(id: string): ExperimentMetadata | null {
     ...
     */
    findExperiments(filter: ExperimentFilter = {}): ExperimentMetadata[] {
        return Object.values(this.experimentsCache).filter(exp => {
            ...
            if (filter.metrics && exp.metrics) {
                for (const [metricName, range] of Object.entries(filter.metrics)) {
                    const metricValue = exp.metrics[metricName as OptimizationMetric];
     ...
     */
    compareExperiments(ids: string[]): Record<string, any> {
        ...
        Array.from(allMetrics).forEach(metric => {
            comparison.metrics[metric] = experiments.map(exp => ({
                ...
                value: exp.metrics[metric as OptimizationMetric] || null
            }));
        });
     ...
     */
    generateExperimentReport(id: string): string {
     ...
     */
    private loadExperimentsToCache(): void {
        if (!fs.existsSync(this.experimentsDir)) return;
            ...
            if (item.isDirectory()) {
                ...
                if (fs.existsSync(metadataPath)) {
                    try {
                        ...
                        const metadata = JSON.parse(content) as ExperimentMetadata;
     ...
     */
    private saveExperimentMetadata(metadata: ExperimentMetadata): void {
     ...
     */
    private generateReportHtml(experiment: ExperimentMetadata, iterations: any[]): string {
     ...
     */
    private generateDashboardHtml(experiments: ExperimentMetadata[]): string {
        // Zliczanie statystyk
                            ...
                            <div style="flex: 1;">
                                ...
                                <select id="filter-strategy" style="width: 100%; padding: 8px;">
                                    ...
                                    ${Object.entries(strategyCounts).map(([strategy, count]) => 
                                        `<option value="${strategy}">${strategy} (${count})</option>`
                                    ).join('')}
                                </select>
                            </div>
                            <div style="flex: 1;">
                                ...
                                <select id="filter-profile" style="width: 100%; padding: 8px;">
                                    ...
                                    ${Object.entries(profileCounts).map(([profile, count]) => 
                                        `<option value="${profile}">${profile} (${count})</option>`
                                    ).join('')}
                                </select>
                            </div>
                            <div style="flex: 1;">
                    ...
                    <table class="experiments-table">
                        ...
                        <tbody>
                            ...
                            <tr data-id="${exp.id}" class="experiment-row">
                                ...
                                <td>${exp.status === 'completed' && exp.metrics[exp.profileSettings.primaryMetric as OptimizationMetric] !== undefined
                                    ? exp.metrics[exp.profileSettings.primaryMetric as OptimizationMetric]?.toFixed(4)
                                    : 'N/A'}</td>
                                <td>
