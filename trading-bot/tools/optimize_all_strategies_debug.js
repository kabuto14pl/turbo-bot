...
// Logowanie startowe
logDebug('=====================================================');
logDebug('ROZPOCZYNAM DZIAŁANIE SKRYPTU OPTYMALIZACJI (DEBUG)');
logDebug('=====================================================');
// Konfiguracja
...
function runSimpleRSIStrategy(candles, params) {
...
function runSimpleSuperTrendStrategy(candles, params) {
...
async function optimizeAllStrategies() {
    logDebug('Rozpoczynam główną funkcję optymalizacji (DEBUG)');
    // Definicja plików dla różnych interwałów
    const timeframes = [
        { name: '15m', file: 'BTC_data_15m.csv' },
        { name: '1h', file: 'BTC_data_1h.csv' }
    ];
    ...
    for (const timeframe of timeframes) {
        try {
            ...
            const rawCandles = await (0, csv_loader_1.loadCandles)(timeframe.file);
    ...
    const resultsDir = path.resolve(__dirname, `../results/optimization_debug_${timestamp}`);
    ...
    <html>
    <head>
        <title>Raport Optymalizacji - Debug</title>
        <style>
    ...
    <body>
        <h1>Raport Optymalizacji Strategii (Debug)</h1>
