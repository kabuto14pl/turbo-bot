...
import { DuckDBIntegration } from './duckdb_integration';
...
export interface DrawdownPeriod {
    ...
    max_drawdown: number;
    recovery_timestamp: number | null;
}
...
export class QueryBuilder {
    constructor(private db: DuckDBIntegration) {}
    ...
    async getPerformanceByPeriod(
        ...
        return await (this.db as any).query(query);
    }
     ...
     */
    async getDrawdownPeriods(threshold: number = 0.05): Promise<DrawdownPeriod[]> {
        const query = `
     ...
     */
    async getWinLossStreaks(minStreakLength: number = 3): Promise<WinLossStreak[]> {
        const query = `
     ...
     */
    async getTimeWeightedReturns(days: number = 30): Promise<number> {
     ...
     */
    async getHourlyTradeDistribution(): Promise<any[]> {
        const query = `
            SELECT 
     ...
     */
    async getStrategyCorrelation(): Promise<any[]> {
        const query = `
     ...
     */
    async getMonthlyPerformanceSummary(): Promise<any[]> {
        const query = `
            SELECT 
     ...
     */
    async getRiskAdjustedMetrics(days: number = 30): Promise<any> {
        const cutoffTime = Date.now() - (days * 24 * 60 * 60 * 1000);
