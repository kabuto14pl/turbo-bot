...
describe('Helpers Module', () => {
    beforeEach(() => {
        ...
        helpers_1.signal_events.length = 0;
        helpers_1.trade_events.length = 0;
    });
    describe('Event Recording', () => {
        it('should record signal events correctly', () => {
            (0, helpers_1.record_signal)('RSITurbo', Date.now(), 'BUY', 50000, { confidence: 0.8 });
            expect(helpers_1.signal_events).toHaveLength(1);
            expect(helpers_1.signal_events[0].strategy).toBe('RSITurbo');
            expect(helpers_1.signal_events[0].type).toBe('BUY');
            expect(helpers_1.signal_events[0].price).toBe(50000);
            expect(helpers_1.signal_events[0].confidence).toBe(0.8);
        });
        it('should record trade events correctly', () => {
            (0, helpers_1.record_trade)('SuperTrend', 'OPEN', 'buy', Date.now(), 45000, 0.1, 44000, 47000, null);
            expect(helpers_1.trade_events).toHaveLength(1);
            expect(helpers_1.trade_events[0].strategy).toBe('SuperTrend');
            expect(helpers_1.trade_events[0].action).toBe('OPEN');
            expect(helpers_1.trade_events[0].side).toBe('buy');
            expect(helpers_1.trade_events[0].price).toBe(45000);
            expect(helpers_1.trade_events[0].size).toBe(0.1);
        });
    });
    describe('CSV Export', () => {
        it('should convert data to CSV format correctly', () => {
            ...
            const csv = (0, helpers_1.toCsv)(data, columns);
    ...
    describe('Market Analysis Functions', () => {
        it('should calculate volatility correctly', () => {
            ...
            const volatility = (0, helpers_1.calculateVolatility)(prices);
        ...
        it('should determine bullish trend correctly', () => {
            ...
            const trend = (0, helpers_1.determineTrend)(prices);
    ...
    describe('Data Validation', () => {
        it('should validate complete candles successfully', () => {
            ...
            const isValid = (0, helpers_1.validateCandles)(candles);
    ...
    describe('Timestamp Utilities', () => {
        it('should format timestamp to ISO string', () => {
            ...
            const formatted = (0, helpers_1.formatTimestamp)(timestamp);
        ...
        it('should get timestamp range from candles', () => {
            ...
            const range = (0, helpers_1.getTimestampRange)(candles);
    ...
    describe('Performance Calculations', () => {
        it('should calculate Sharpe ratio correctly', () => {
            ...
            const sharpe = (0, helpers_1.calculateSharpeRatio)(returns, 0);
            expect(typeof sharpe).toBe('number');
            expect(sharpe).toBeGreaterThan(0);
        });
        it('should return 0 Sharpe ratio for empty returns', () => {
            const returns = [];
            const sharpe = (0, helpers_1.calculateSharpeRatio)(returns);
            expect(sharpe).toBe(0);
        });
        it('should calculate max drawdown correctly', () => {
            const equityCurve = [1000, 1100, 1050, 900, 950, 1200];
            const maxDD = (0, helpers_1.calculateMaxDrawdown)(equityCurve);
