...
class EnsembleVoting {
    constructor() {
        ...
        this.staticWeights = {
            // PATCH #42C: Only 5 classical strategies vote (ML is ADVISOR)
            'AdvancedAdaptive': 0.27, 'RSITurbo': 0.18, 'SuperTrend': 0.22,
            'MACrossover': 0.17, 'MomentumPro': 0.16,
        };
    ...
    vote(signals, riskManager, mtfBias) {
        ...
        // Dynamic threshold
        const hasConflict = votes.BUY > 0.25 && votes.SELL > 0.25; // PATCH #41B: raised from 0.15 to reduce false conflicts
        ...
        // PATCH #42: ML override moved to bot.js ML Advisor gate
        ...
        if (mtfBias && action !== 'HOLD') {
            ...
            // PATCH #26: Stronger counter-trend blocking ÔÇö analysis showed 0% WR on counter-trend trades
            if (counter && mtfBias.confluence && mtfBias.confluence.allAligned) {
                ...
                console.log('   [MTF P26] STRONG PENALTY: ' + action + ' vs ' + mtfDir + ' bias (conf *0.35 ÔÇö P26)');
            } else if (aligned) {
                weightedConf *= mtfMul;
        ...
        // PATCH #23b+#26: NEURON AI HOLD Override ÔÇö require score >= 25 (was 20)
        if (action === 'HOLD' && mtfBias && Math.abs(mtfBias.score) >= 25) {
            ...
            if (mlFound) {
                ...
                console.log('[NEURON AI] Ja, NEURON AI, przejmuj─Ö kontrole: ' + mtfDir + ' | HOLD consensus odrzucony');
