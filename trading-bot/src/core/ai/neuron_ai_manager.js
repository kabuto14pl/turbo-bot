    ...
    _fallbackDecision(state) {
        ...
        if (consecutiveLosses >= 3 && action !== 'HOLD' && action !== 'CLOSE') {
            ...
            if (mtfScore < 25) {
                ...
                confidence *= 0.70; // Reduce confidence by 30%
                reason += ' | WARNING: ' + consecutiveLosses + ' losses streak -- confidence reduced';
            }
        }
...
// Moderate signals: follow ensemble majority if clear (multi-position OK)
    ...
    _buildPrompt(state) {
        ...
        // PATCH #26: Enhanced rules with anti-scalping and quality focus
        prompt += 'Zasady: Mozesz otwierac wiele pozycji jednoczesnie (max 5). Mozesz SCALE_IN, OPEN_SHORT, FLIP.\n';
        prompt += 'ANTI-SCALPING: Minimum oczekiwany hold = 30-60 minut. NIE otwieraj pozycji ktore zamkniesz w <15 min.\n';
        prompt += 'QUALITY: Lepiej 3 dobre trade/dzien niz 15 slabych. Fees = ~$3.59/trade = glowny zabojca zysku.\n';
