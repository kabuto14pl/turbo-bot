...
import { MarketState, PerformanceMetrics } from './types';

interface EnsembleConfig {
  // Model selection
  enabled_models: ModelType[];
  min_models_required: number;
  
  // Voting strategy
  voting_strategy: 'weighted' | 'majority' | 'confidence_based' | 'adaptive';
  confidence_threshold: number;
  
  // Weight management
  auto_adjust_weights: boolean;
  weight_adjustment_interval: number; // milliseconds
  performance_lookback_window: number; // number of predictions
  
  // Model health
  health_check_enabled: boolean;
  min_model_accuracy: number;
  max_model_error_rate: number;
  auto_disable_unhealthy: boolean;
  
  // Performance tracking
  track_individual_performance: boolean;
  track_ensemble_performance: boolean;
  performance_history_size: number;
}

type ModelType = 'deep_rl' | 'xgboost' | 'lstm' | 'transformer' | 'cnn' | 'random_forest';
...
interface EnsemblePrediction {
  ...
  model_weights: Map<ModelType, number>;
  models_agreed: number;
  total_models: number;
  
  // Voting details
  voting_strategy_used: string;
  buy_votes: number;
  sell_votes: number;
  hold_votes: number;
...
interface EnsemblePerformance {
  ...
  ensemble_win_rate: number;
  
  // Individual model performance
  model_performances: Map<ModelType, ModelMetrics>;
  
  // Comparative analysis
  best_performing_model: ModelType;
  worst_performing_model: ModelType;
  model_correlation_matrix: number[][];
...
export class EnsemblePredictionEngine {
  ...
  private modelWeights: Map<ModelType, number> = new Map();
  ...
  private weightAdjustmentTimer?: NodeJS.Timeout;
  private lastWeightUpdate: number = 0;
  ...
  constructor(config: Partial<EnsembleConfig> = {}) {
    ...
    this.logger = {
      info: (msg: string) => console.log(`[ENSEMBLE] ${msg}`),
      warn: (msg: string) => console.warn(`[ENSEMBLE] ${msg}`),
      error: (msg: string) => console.error(`[ENSEMBLE] ${msg}`)
    };
  }
  ...
  async predict(marketState: MarketState): Promise<EnsemblePrediction> {
    ...
    if (predictions.length < this.config.min_models_required) {
      throw new Error(
        `Insufficient models for prediction: ${predictions.length}/${this.config.min_models_required}`
      );
    }
   ...
   */
  private weightedVoting(predictions: ModelPrediction[]): {
    action: 'BUY' | 'SELL' | 'HOLD';
    confidence: number;
  } {
    ...
    const action = Object.keys(normalizedWeights).find(
      key => normalizedWeights[key as keyof typeof normalizedWeights] === maxWeight
    ) as 'BUY' | 'SELL' | 'HOLD';
   ...
   */
  private majorityVoting(predictions: ModelPrediction[]): {
    action: 'BUY' | 'SELL' | 'HOLD';
    confidence: number;
  } {
    ...
    const action = Object.keys(votes).find(
      key => votes[key as keyof typeof votes] === maxVotes
    ) as 'BUY' | 'SELL' | 'HOLD';
   ...
   */
  private confidenceBasedVoting(predictions: ModelPrediction[]): {
    action: 'BUY' | 'SELL' | 'HOLD';
    confidence: number;
  } {
    // Filter predictions by confidence threshold
    const highConfidencePredictions = predictions.filter(
      p => p.confidence >= this.config.confidence_threshold
    );
   ...
   */
  private adaptiveVoting(predictions: ModelPrediction[]): {
    action: 'BUY' | 'SELL' | 'HOLD';
    confidence: number;
  } {
  ...
  updatePredictionOutcome(
    ...
    if (
      this.config.auto_adjust_weights &&
   ...
   */
  getPerformanceReport(): EnsemblePerformance {
    const modelPerformances = new Map(this.modelMetrics);
    
    // Find best and worst models
    let bestModel: ModelType = 'deep_rl';
    let worstModel: ModelType = 'deep_rl';
    ...
    const unanimousDecisions = recentPredictions.filter(
      p => p.models_agreed === p.total_models
    ).length;
   ...
   */
  private generateSyntheticAction(marketState: MarketState, modelType: ModelType): 'BUY' | 'SELL' | 'HOLD' {
    ...
    if (rand < modelBias - 0.2) return 'SELL';
    if (rand > modelBias + 0.2) return 'BUY';
    return 'HOLD';
  }
   ...
   */
  private createDefaultMetrics(modelType: ModelType): ModelMetrics {
    return {
      model_type: modelType,
      total_predictions: 0,
   ...
   * 📝 Track prediction
   */
  private trackPrediction(prediction: EnsemblePrediction): void {
   ...
   */
  private updateModelError(modelType: ModelType): void {
    const metrics = this.modelMetrics.get(modelType);
    if (metrics) {
      metrics.error_count++;
   ...
   */
  private getWeightsSummary(): string {
   ...
   */
  private calculateEnsembleAccuracy(): number {
    // Placeholder - would calculate from actual outcomes
    return 0.65;
  }
  
  private calculateEnsembleSharpe(): number {
    // Placeholder
    return 1.5;
  }
  
  private calculateEnsembleWinRate(): number {
    // Placeholder
    return 0.58;
  }
  ...
  private startWeightAdjustment(): void {
    this.weightAdjustmentTimer = setInterval(
      () => this.adjustWeights(),
      this.config.weight_adjustment_interval
    );
  }
