...
class AdvancedExperienceBuffer {
    ...
    /**
     * 📊 GET BUFFER STATUS
     * Returns current status of the experience buffer
     */
    getStatus() {
        ...
        return {
            ...
            isFull: this.isFull,
            totalExperiences: this.totalSamples,
            averageReward: avgReward,
            prioritizedReplay: this.config?.prioritizedReplay || true
        };
    }
    /**
     * 📤 EXPORT EXPERIENCES
     * Export all experiences for analysis or backup
     */
    exportExperiences() {
        return [...this.buffer];
    }
    /**
     * 🔄 RESET BUFFER
     * Clear all experiences and reset statistics
     */
    reset() {
        this.buffer = [];
        this.priorities = new Float32Array(this.maxSize);
        this.priorityWeights = new Float32Array(this.maxSize);
        this.isFull = false;
        this.totalSamples = 0;
        this.addCount = 0;
        this.sampleCount = 0;
        ...
        this.maxPriority = 1.0;
        this.sumPriorities = 0;
        this.logger.info('🔄 Experience buffer reset');
    }
    /**
     * 🧹 DISPOSE
     * Clean up resources
    ...
    dispose() {
        this.reset();
        this.logger.info('🧹 Experience buffer disposed');
    }
}
