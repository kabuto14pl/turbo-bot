...
interface BacktestConfig {
  ...
  training_window_days: number; // In-sample period
  testing_window_days: number; // Out-of-sample period
  anchored: boolean; // If true, always start from beginning
  
  // Monte Carlo settings
  monte_carlo_enabled: boolean;
  num_simulations: number;
  confidence_levels: number[]; // e.g., [0.05, 0.25, 0.50, 0.75, 0.95]
  
  // Transaction costs
  commission_rate: number; // Per trade percentage
  slippage_model: 'fixed' | 'proportional' | 'market_impact';
  fixed_slippage_bps: number; // Basis points
  market_impact_coefficient: number; // For large orders
  
  // Execution modeling
  execution_delay_ms: number; // Latency simulation
  partial_fills_enabled: boolean;
  min_fill_probability: number;
  
  // Regime detection
  regime_detection_enabled: boolean;
  regime_lookback_days: number;
  volatility_threshold: number;
  trend_threshold: number;
  
  // Performance tracking
  track_drawdowns: boolean;
  track_equity_curve: boolean;
...
interface BacktestResult {
  ...
  drawdown_curve: { timestamp: number; drawdown: number }[];
  trade_log: Trade[];
  ...
  // Regime analysis
  regime_performance?: Map<string, RegimePerformance>;
...
interface WalkForwardResult {
  ...
  // Degradation
  performance_degradation: number; // (in_sample - out_of_sample) / in_sample
  overfitting_score: number; // 0-1 scale, higher = more overfit
...
interface MonteCarloResult {
  ...
  // Percentiles
  percentile_returns: Map<number, number>; // e.g., 5th, 25th, 50th, 75th, 95th
  percentile_sharpe: Map<number, number>;
  percentile_max_drawdown: Map<number, number>;
  
  // Risk of ruin
  probability_of_ruin: number; // Probability of losing >50% capital
  ...
  // Paths
  simulated_paths: { returns: number[]; equity: number[] }[];
}
...
interface RegimePerformance {
  ...
  return_in_regime: number;
  sharpe_in_regime: number;
  win_rate_in_regime: number;
}

/**
 * 🔄 K-FOLD CROSS-VALIDATION RESULT
 * 🚀 FAZA 1.4: Complete enterprise-grade k-fold validation
 */
interface KFoldValidationResult {
  ...
  avg_train_win_rate: number;
  avg_train_trades: number;
  
  // Aggregate testing metrics  
  avg_test_return: number;
  avg_test_sharpe: number;
  avg_test_win_rate: number;
  avg_test_trades: number;
  
  // Overfitting detection
  avg_overfitting_gap: number; // (train - test) / train
  max_overfitting_gap: number;
  min_overfitting_gap: number;
  overfitting_detected: boolean; // true if gap > 20%
  
  // Consistency metrics
  std_dev_test_return: number; // Variance across folds
  std_dev_test_sharpe: number;
...
interface KFoldFold {
  ...
  // Degradation
  overfitting_gap: number; // (train - test) / train
  sharpe_degradation: number; // (train_sharpe - test_sharpe) / train_sharpe
  
  // Strategy parameters (if optimized)
  optimized_params?: Map<string, any>;
}
...
export class AdvancedBacktestEngine {
  ...
  private regimeHistory: { timestamp: number; regime: string }[] = [];
  ...
  constructor(config: Partial<BacktestConfig> = {}) {
    ...
    this.logger = {
      info: (msg: string) => console.log(`[BACKTEST] ${msg}`),
      warn: (msg: string) => console.warn(`[BACKTEST] ${msg}`),
      error: (msg: string) => console.error(`[BACKTEST] ${msg}`)
    };
  }
   ...
   */
  private async runSimpleBacktest(strategy: any): Promise<void> {
  ...
  async performKFoldValidation(
    strategy: any,
    k: number = 5,
    optimize_params: boolean = false
  ): Promise<KFoldValidationResult> {
    const startTime = Date.now();
    ...
    for (let foldId = 0; foldId < k; foldId++) {
      ...
      let optimizedParams: Map<string, any> | undefined;
    ...
    const result: KFoldValidationResult = {
      k,
      folds,
   ...
   */
  private async runBacktestOnTimestamps(
    strategy: any,
    timestamps: number[]
  ): Promise<void> {
    ...
    for (const [symbol, candles] of this.marketData.entries()) {
      ...
      const filtered = candles.filter((c: any) => timestampSet.has(c.timestamp));
   ...
   */
  private async runSimpleBacktestOnData(
    strategy: any,
    data: Map<string, any[]>
  ): Promise<void> {
   ...
   */
  private async runWalkForwardBacktest(strategy: any): Promise<void> {
    ...
    while (currentIndex < allTimestamps.length) {
      ...
      if (trainingEndIndex >= allTimestamps.length) break;
   ...
   */
  private async runMonteCarloSimulation(): Promise<MonteCarloResult> {
    ...
    const simulations: { returns: number[]; equity: number[] }[] = [];
    const finalReturns: number[] = [];
    const finalSharpes: number[] = [];
    ...
    const percentileReturns = new Map<number, number>();
    const percentileSharpe = new Map<number, number>();
    const percentileMaxDD = new Map<number, number>();
   ...
   * 📉 Calculate slippage
   */
  private calculateSlippage(price: number, quantity: number, direction: string): number {
   ...
   */
  private detectRegime(timestamp: number): void {
   ...
   */
  private calculateRegimePerformance(): Map<string, RegimePerformance> {
    const regimePerformance = new Map<string, RegimePerformance>();
  ...
  private getMarketState(timestamp: number): any {
    // Placeholder
    return {
      timestamp,
  ...
  private getRecentData(timestamp: number, days: number): any[] {
    // Placeholder
    return [];
  }
  ...
  private getDataWindow(startIndex: number, endIndex: number): any {
    // Placeholder
    return {};
  }
  
  private async optimizeStrategy(strategy: any, data: any): Promise<Map<string, any>> {
    // Placeholder - would run optimization
    return new Map();
  }
  
  private async evaluateStrategy(strategy: any, data: any): Promise<any> {
    // Placeholder
    return {
      inSampleReturn: 0.1,
      inSampleSharpe: 1.5,
  ...
  private calculateDrawdownCurve(): { timestamp: number; drawdown: number }[] {
    const curve: { timestamp: number; drawdown: number }[] = [];
    let maxEquity = this.config.initial_capital;
  ...
  private calculateVaRFromReturns(returns: number[], confidence: number): number {
    if (returns.length === 0) return 0;
  ...
  private calculateCVaRFromReturns(returns: number[], confidence: number): number {
