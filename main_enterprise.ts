...
import { EnterpriseOptimizedStrategyEngine } from './core/strategies/enterprise_optimized_strategy_engine';

// Trading Bot Core
import { PerformanceTracker } from './trading-bot/core/analysis/performance_tracker';
import { DeepRLAgent } from './trading-bot/src/core/ml/deep_rl_agent';
...
class TurboBotEnterpriseServer {
  ...
  // Enterprise Performance Components
  private performanceTracker!: PerformanceTracker;
  private enterprisePerformanceManager!: IntegratedPerformanceManager;
  private realTimeRiskMonitoring!: RealTimeRiskMonitoring;
  
  // Enterprise Strategy Engine
  private strategyEngine!: EnterpriseOptimizedStrategyEngine;
  
  // ML Components
  private deepRLAgent?: DeepRLAgent;
  ...
  constructor() {
    ...
    const strategyEngine = new EnterpriseOptimizedStrategyEngine();
    const activeStrategies = strategyEngine.getActiveStrategies();
    
    this.tradingState = {
      isRunning: false,
      mode: config.tradingMode as 'demo' | 'backtest' | 'production',
  ...
  private async initializeEnterpriseComponents(): Promise<void> {
    try {
      ...
      this.strategyEngine = new EnterpriseOptimizedStrategyEngine();
      ...
      this.enterprisePerformanceManager = new IntegratedPerformanceManager(this.performanceTracker);
      ...
      this.realTimeRiskMonitoring = new RealTimeRiskMonitoring(this.enterprisePerformanceManager);
      ...
      if (config.enableML) {
        ...
        this.deepRLAgent = new DeepRLAgent(mlConfig);
...
export default TurboBotEnterpriseServer;
