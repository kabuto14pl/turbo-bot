...
exports.MonitoringSystemTestSuite = void 0;
const monitoring_system_integration_1 = require("./monitoring_system_integration");
class MonitoringSystemTestSuite {
    constructor() {
        this.testResults = new Map();
        this.startTime = 0;
        this.mockExternalSystems = {};
        // Use test configuration
        const testConfig = {
            ...monitoring_system_integration_1.DefaultMonitoringConfig,
            prometheus: { ...monitoring_system_integration_1.DefaultMonitoringConfig.prometheus, port: 9091 }, // Different port for testing
            notifications: { ...monitoring_system_integration_1.DefaultMonitoringConfig.notifications, port: 8081 }
        };
        this.integration = new monitoring_system_integration_1.MonitoringSystemIntegration(testConfig);
    ...
    async testRealTimeMetricsCollection() {
        const startTime = Date.now();
        try {
    ...
    async testPrometheusExportFormat() {
        const startTime = Date.now();
        try {
            const integration = this.integration;
            const prometheus = integration.prometheusExporter;
            ...
            const hasMetricLines = formatted.split('\n').some((line) => line && !line.startsWith('#') && line.includes(' '));
    ...
    async testAlertGenerationSpeed() {
        const startTime = Date.now();
        try {
            const integration = this.integration;
            const prometheus = integration.prometheusExporter;
    ...
    async testAlertNotificationDelivery() {
        const startTime = Date.now();
        try {
            const integration = this.integration;
            const notifications = integration.notificationSystem;
    ...
    async runMonitoringCoverageTests() {
        const suite = {
    ...
    async testSystemComponentCoverage() {
        const startTime = Date.now();
        try {
            ...
            const monitoredComponents = components.filter(component => {
                const status = integrationStatus[component];
    ...
    async testCriticalMetricsCoverage() {
        const startTime = Date.now();
        try {
            const integration = this.integration;
            const prometheus = integration.prometheusExporter;
    ...
    async testIntegrationCoverage() {
        ...
        try {
            ...
            const activeIntegrations = expectedIntegrations.filter(integration => {
                return integrationStatus.externalSystems[integration];
            });
    ...
    async testAlertThresholdAccuracy() {
        const startTime = Date.now();
        try {
            const integration = this.integration;
            const prometheus = integration.prometheusExporter;
    ...
    async analyzeTestResults() {
        ...
        for (const [suiteName, suite] of this.testResults) {
            ...
            // Update requirements
            if (suite.requirements.realTimeMetrics)
                requirements.realTimeMetrics = true;
            if (suite.requirements.alertResponseTime)
                requirements.alertResponseTime = true;
            if (suite.requirements.monitoringCoverage)
                requirements.monitoringCoverage = true;
            if (suite.requirements.falsePositives)
                requirements.falsePositives = true;
        }
...
exports.MonitoringSystemTestSuite = MonitoringSystemTestSuite;
