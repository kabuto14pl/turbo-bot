...
exports.DefaultNotificationConfig = exports.RealTimeNotificationSystem = void 0;
const events_1 = require("events");
const WebSocket = __importStar(require("ws"));
const http = __importStar(require("http"));
const nodemailer = __importStar(require("nodemailer"));
const axios_1 = __importDefault(require("axios"));
class RealTimeNotificationSystem extends events_1.EventEmitter {
    ...
    async testSlackIntegration(config) {
        ...
        await axios_1.default.post(config.webhookUrl, testMessage, {
            timeout: 5000,
    ...
    async processMessage(message) {
        ...
        const sendPromises = targetChannels.map(channelId => this.sendToChannel(channelId, message));
        try {
    ...
    messagePassesFilters(message, filters) {
        return filters.every(filter => {
            const value = message[filter.property];
    ...
    async sendToWebSocketClients(message) {
        const connectedClients = Array.from(this.clients.values()).filter(client => client.socket.readyState === WebSocket.OPEN &&
    ...
    async sendToSlack(config, message) {
        const color = {
            critical: 'danger',
            warning: 'warning',
            info: 'good'
        ...
        await axios_1.default.post(config.webhookUrl, slackMessage, {
            timeout: 10000,
            headers: { 'Content-Type': 'application/json' }
        });
    }
    async sendToEmail(config, message) {
    ...
    async sendToPagerDuty(config, message) {
        const payload = {
    ...
    async sendToWebhook(config, message) {
        await axios_1.default.post(config.url, message, {
...
exports.RealTimeNotificationSystem = RealTimeNotificationSystem;
