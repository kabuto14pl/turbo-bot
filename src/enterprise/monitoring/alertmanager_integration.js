...
class AlertManagerIntegration extends events_1.EventEmitter {
    ...
    async initialize() {
        try {
            ...
            console.log('[ALERTMANAGER] ✅ Integration initialized successfully');
    ...
    async testConnections() {
        try {
            ...
            console.log('[ALERTMANAGER] ✅ AlertManager connection successful');
            // Test Prometheus
            const promResponse = await this.prometheusClient.get('/-/healthy');
            console.log('[ALERTMANAGER] ✅ Prometheus connection successful');
        }
        catch (error) {
            console.error('[ALERTMANAGER] ❌ Connection test failed:', error.message);
    ...
    async deployAlertRules() {
        try {
            ...
            if (!validation.valid) {
                console.error('[ALERTMANAGER] ❌ Alert rules validation failed:', validation.errors);
                return false;
            }
            ...
            console.log('[ALERTMANAGER] ✅ Alert rules deployed successfully');
            this.emit('rulesDeployed');
            return true;
        }
        catch (error) {
            console.error('[ALERTMANAGER] ❌ Failed to deploy alert rules:', error);
            return false;
        }
    }
    ...
    handleNewAlert(alert) {
        ...
        console.warn(`[ALERTMANAGER] 🚨 New alert: ${alertName} (${severity})`);
    ...
    handleResolvedAlert(alert) {
        const alertName = alert.labels.alertname || 'unknown';
        console.log(`[ALERTMANAGER] ✅ Alert resolved: ${alertName}`);
