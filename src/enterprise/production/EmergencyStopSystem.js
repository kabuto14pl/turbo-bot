...
class EmergencyStopSystem extends events_1.EventEmitter {
    constructor(portfolio, riskLimits) {
        super();
     ...
     */
    async triggerEmergencyStop(level, reason, manualOperator) {
     ...
     */
    testCircuitBreaker(level, triggerType) {
     ...
     */
    async attemptRecovery(manualOverride = false) {
     ...
     */
    updateRiskAssessment(varResult) {
     ...
     */
    async forceLiquidatePosition(positionId, urgency = 'FAST') {
            ...
            for (const [level, breaker] of Array.from(this.circuitBreakers)) {
                ...
                for (const trigger of breaker.triggers) {
                    ...
                    if (shouldTrigger.triggered) {
                        this.executeEmergencyStop(level, trigger.type, shouldTrigger.value, trigger.threshold, trigger.description);
    ...
    evaluateLossPercentage(trigger) {
    ...
    evaluatePositionSize(trigger) {
        const maxPositionValue = Math.max(...this.portfolio.positions.map(p => Math.abs(p.size) * p.currentPrice));
    ...
    evaluateLiquidity(trigger) {
        // Simplified liquidity check - would need real market data
        const totalPositionValue = this.portfolio.positions.reduce((sum, p) => sum + Math.abs(p.size) * p.currentPrice, 0);
    ...
    async executeEmergencyStop(level, triggerType, triggerValue, threshold, description, metadata) {
        const breaker = this.circuitBreakers.get(level);
        if (!breaker) {
    ...
    async liquidateAllPositions(urgency) {
    ...
    async notifyRiskManagement(event) {
        const notification = {
    ...
    async validateRecoveryConditions(manualOverride) {
    ...
    async executeRecovery() {
        ...
        this.liquidationQueue = this.liquidationQueue.filter(order => order.status === 'EXECUTING' || order.status === 'PENDING');
    ...
    handleLiquidationCompleted(order) {
