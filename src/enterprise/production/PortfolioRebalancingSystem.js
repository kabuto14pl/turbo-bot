...
class PortfolioRebalancingSystem extends events_1.EventEmitter {
    constructor(portfolio, config) {
        super();
        this.allocationTargets = new Map();
    ...
    startMonitoring(intervalMs = 60000) {
        if (this.isMonitoring) {
            console.log('⚠️ Portfolio rebalancing monitoring already active');
            return;
        }
        console.log('⚖️ Starting Portfolio Rebalancing monitoring...');
        this.monitoringInterval = setInterval(async () => {
            await this.performRebalancingCheck();
        }, intervalMs);
        this.isMonitoring = true;
        this.emit('monitoringStarted');
        console.log('✅ Portfolio rebalancing monitoring active');
    }
    /**
     * Stop monitoring
     */
    stopMonitoring() {
        ...
        console.log('🛑 Portfolio rebalancing monitoring stopped');
    }
    /**
     * Set target allocations
     */
    setTargetAllocations(targets) {
        console.log('🎯 Setting target allocations...');
        ...
        console.log(`✅ Set ${targets.length} allocation targets`);
    }
    /**
    ...
    analyzeAllocationDrift() {
        ...
        for (const [symbol, target] of Array.from(this.allocationTargets)) {
     ...
     */
    async executeRebalancing(reason = 'Manual rebalancing') {
        ...
        try {
            ...
            console.log(`✅ Rebalancing completed: ${trades.length} trades executed`);
     ...
     */
    async optimizeAllocations(riskMetrics, varResult) {
        console.log('🔬 Optimizing allocations based on risk metrics...');
        ...
        if (adjustments.length > 0) {
            console.log(`📊 Generated ${adjustments.length} risk-based allocation adjustments`);
    ...
    performCorrelationAnalysis() {
        ...
        const recommendedAdjustments = new Map();
     ...
     * Update market data
     */
    updateMarketData(symbol, price, volatility) {
    ...
    async performRebalancingCheck() {
        try {
            ...
            if (significantDrifts.length > 0) {
                console.log(`📊 Detected ${significantDrifts.length} significant allocation drifts`);
                ...
                if (criticalDrifts.length > 0) {
                    console.log('🚨 Critical drift detected - triggering emergency rebalancing');
                ...
                // Auto-rebalance for high drifts
                    ...
                    console.log('⚠️ High drift detected - triggering automatic rebalancing');
    ...
    async planRebalancingTrades(drifts) {
        const trades = [];
        const totalPortfolioValue = this.portfolio.totalValue;
        let tradeId = 0;
        ...
        if (costPct > this.config.maxTransactionCost) {
            ...
            console.log(`💰 Filtered trades: ${trades.length} -> ${filteredTrades.length} to stay within cost limit`);
            return filteredTrades;
        }
        return trades;
    }
    async executeRebalancingTrades(rebalancingEvent) {
        rebalancingEvent.status = 'EXECUTING';
        const startTime = Date.now();
        console.log(`⚡ Executing ${rebalancingEvent.plannedTrades.length} rebalancing trades...`);
        for (const trade of rebalancingEvent.plannedTrades) {
            try {
                ...
                console.log(`✅ Trade completed: ${trade.action} ${trade.actualQuantity} ${trade.symbol} @ $${trade.actualPrice?.toFixed(2)}`);
            }
            catch (error) {
                trade.status = 'FAILED';
                console.error(`❌ Trade failed: ${trade.symbol}`, error);
        ...
        console.log(`📊 Rebalancing execution completed in ${rebalancingEvent.executionTime}ms`);
        console.log(`💰 Total transaction costs: $${rebalancingEvent.transactionCosts.toFixed(2)}`);
    }
    ...
    updatePortfolioFromTrade(symbol, action, quantity, price) {
    ...
    estimateTransactionCost(quantity, price) {
        const tradeValue = quantity * price;
    ...
    calculateTradePriority(drift, target) {
    ...
    calculateRebalancingPerformance(event) {
    ...
    calculateAdjustmentConfidence(symbol, reasons) {
        ...
        if (reasons.includes('VOLATILITY'))
            confidence += 0.2;
        if (reasons.includes('VAR'))
            confidence += 0.2;
        if (reasons.includes('CONCENTRATION'))
            confidence += 0.1;
    ...
    getRecommendedTaxAction(loss, holdingPeriod) {
        if (holdingPeriod < 30)
            return 'DEFER'; // Avoid wash sale
        if (loss > 10000)
            return 'HARVEST'; // Large loss
        if (loss > 1000)
            return 'PARTIAL'; // Medium loss
    ...
    calculateDiversificationRatio(correlationMatrix, symbols) {
    ...
    handleDriftDetected(drifts) {
        console.log(`📊 Allocation drift detected: ${drifts.length} assets out of target`);
        for (const drift of drifts) {
            if (drift.severity === 'CRITICAL') {
                console.log(`🚨 CRITICAL: ${drift.symbol} drift: ${(drift.drift * 100).toFixed(1)}%`);
            }
        }
    }
}
