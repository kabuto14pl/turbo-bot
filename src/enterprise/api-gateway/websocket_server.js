...
exports.EnterpriseWebSocketServer = void 0;
const events_1 = require("events");
const ws_1 = __importDefault(require("ws"));
const uuid_1 = require("uuid");
class EnterpriseWebSocketServer extends events_1.EventEmitter {
    constructor(config, authManager) {
        super();
        this.connections = new Map();
        this.channels = new Map(); // channel -> connection IDs
    ...
    async start(server) {
        ...
        try {
            ...
            this.wss = new ws_1.default.Server(wsOptions);
    ...
    checkRateLimit(connection) {
        ...
        connection.rateLimitBucket.tokens = Math.min(this.config.rateLimit.burstLimit, connection.rateLimitBucket.tokens + tokensToAdd);
    ...
    async handleAuthMessage(connectionId, message) {
        const connection = this.connections.get(connectionId);
        if (!connection)
            return;
        try {
    ...
    async handleSubscribeMessage(connectionId, message) {
        const connection = this.connections.get(connectionId);
        if (!connection)
            return;
        ...
        for (const channel of channels) {
            ...
            if (connection.user && requiredPermissions.length > 0) {
                const hasPermission = requiredPermissions.some(perm => this.authManager.hasPermission(connection.user, perm));
                if (!hasPermission) {
    ...
    async handleUnsubscribeMessage(connectionId, message) {
        const connection = this.connections.get(connectionId);
        if (!connection)
            return;
    ...
    handlePingMessage(connectionId, message) {
        this.sendMessage(connectionId, {
            id: (0, uuid_1.v4)(),
            type: 'pong',
            payload: {
    ...
    sendErrorMessage(connectionId, errorType, errorMessage) {
        this.sendMessage(connectionId, {
            id: (0, uuid_1.v4)(),
            type: 'error',
            payload: {
    ...
    async broadcastToChannel(channelId, message, excludeConnectionId) {
    ...
    async broadcastToAll(message, filter) {
        const connections = Array.from(this.connections.entries());
        let sentCount = 0;
        for (const [connectionId, connection] of connections) {
            if (filter?.excludeConnectionId && connectionId === filter.excludeConnectionId)
                continue;
            if (filter?.authenticated !== undefined && connection.authenticated !== filter.authenticated)
                continue;
            if (filter?.permissions && connection.user) {
                const hasPermission = filter.permissions.some(perm => this.authManager.hasPermission(connection.user, perm));
                if (!hasPermission)
                    continue;
            }
            this.sendMessage(connectionId, message);
    ...
    startCleanup() {
        this.cleanupInterval = setInterval(() => {
            ...
            for (const [connectionId, connection] of Array.from(this.connections.entries())) {
                if (connection.socket.readyState === ws_1.default.CLOSED ||
                    connection.socket.readyState === ws_1.default.CLOSING) {
    ...
    async kickUser(userId, reason = 'Kicked by admin') {
        const userConnections = this.getUserConnections(userId);
        for (const connectionId of userConnections) {
...
exports.EnterpriseWebSocketServer = EnterpriseWebSocketServer;
