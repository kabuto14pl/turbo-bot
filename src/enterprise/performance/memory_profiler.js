...
exports.EnterpriseMemoryProfiler = void 0;
const v8 = __importStar(require("v8"));
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
class EnterpriseMemoryProfiler {
    constructor(snapshotInterval = 30000, // 30 seconds
    alertCallback) {
    ...
    handleGCEvent(entry) {
        ...
        if (this.gcMetrics.gcPressure > this.thresholds.gcPressure) {
            this.createAlert('critical', 'High GC pressure detected', 'gcPressure', this.gcMetrics.gcPressure, this.thresholds.gcPressure);
        }
    }
    startProfiling() {
        setInterval(() => {
    ...
    checkThresholds(snapshot) {
        ...
        if (heapUsageRatio > this.thresholds.heapUsage) {
            this.createAlert('warning', 'High heap usage', 'heapUsage', heapUsageRatio, this.thresholds.heapUsage);
        }
        // Check RSS limit
        if (snapshot.rss > this.thresholds.rssLimit) {
            this.createAlert('critical', 'RSS memory limit exceeded', 'rss', snapshot.rss, this.thresholds.rssLimit);
        }
        // Check for memory leaks
        snapshot.leaks.forEach(leak => {
            if (leak.severity === 'critical' || leak.severity === 'high') {
                this.createAlert('critical', `Memory leak detected: ${leak.type}`, leak.type, leak.size, this.thresholds.memoryGrowth);
            }
        });
    }
    createAlert(level, message, metric, value, threshold) {
        const alert = {
            level,
            message,
            metric,
...
exports.EnterpriseMemoryProfiler = EnterpriseMemoryProfiler;
