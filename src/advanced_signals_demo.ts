/**
 * 🧪 [TESTING-FRAMEWORK]
 * Testing framework component
 */
/**
 * 🔧 [SHARED-INFRASTRUCTURE]
 * Shared infrastructure component
 */
/**
 * 🚀 ADVANCED REAL-TIME TRADING SIGNALS - DEMO & TEST
 * 
 * Demonstration i testy systemu sygnałów w czasie rzeczywistym
 */

import { AdvancedRealTimeTradingSignals, TradingSignal } from './advanced_realtime_trading_signals';

async function runAdvancedSignalsDemo(): Promise<void> {
    console.log('🚀 Advanced Real-Time Trading Signals Demo');
    console.log('='.repeat(60));

    try {
        // Create signal system
        const signalSystem = new AdvancedRealTimeTradingSignals({
            symbols: ['BTCUSDT', 'ETHUSDT', 'ADAUSDT', 'SOLUSDT'],
            reconnectDelay: 3000
        });

        // Set up event listeners
        signalSystem.on('signal', (signal: TradingSignal) => {
            console.log(`\n🎯 NEW SIGNAL GENERATED:`);
            console.log(`📊 Symbol: ${signal.symbol}`);
            console.log(`📈 Direction: ${signal.direction}`);
            console.log(`💪 Strength: ${(signal.strength * 100).toFixed(1)}%`);
            console.log(`🎖️ Confidence: ${(signal.confidence * 100).toFixed(1)}%`);
            console.log(`⏰ Timeframe: ${signal.timeframe}`);
            console.log(`🔍 Source: ${signal.source}`);
            console.log(`⚠️ Risk Level: ${signal.metadata.riskLevel}`);
            console.log(`💰 Expected Return: ${(signal.metadata.expectedReturn * 100).toFixed(2)}%`);
            
            if (signal.metadata.indicators) {
                console.log(`📋 Indicators:`, Object.keys(signal.metadata.indicators).length, 'metrics');
            }
        });

        signalSystem.on('started', () => {
            console.log('\n✅ Advanced Trading Signals System STARTED!');
            console.log('📡 WebSocket connections active');
            console.log('🧠 ML models loaded and ready');
            console.log('🎯 Signal generation active');
        });

        signalSystem.on('stopped', () => {
            console.log('\n🛑 Advanced Trading Signals System STOPPED');
        });

        // Start the system
        console.log('\n🔄 Starting Advanced Real-Time Trading Signals...');
        await signalSystem.start();

        // Monitor system for 2 minutes
        console.log('\n📊 Monitoring signals for 2 minutes...');
        
        let signalCount = 0;
        const originalSignalHandler = signalSystem.listenerCount('signal');
        
        signalSystem.on('signal', () => signalCount++);

        setTimeout(() => {
            console.log('\n📈 DEMO SUMMARY:');
            console.log(`🎯 Total signals generated: ${signalCount}`);
            console.log(`📊 Active signals: ${signalSystem.getActiveSignals().length}`);
            console.log(`🔧 System active: ${signalSystem.isSystemActive()}`);
            
            // Show performance metrics
            const performance = signalSystem.getPerformanceMetrics();
            console.log(`📋 Signal sources tracked: ${performance.size}`);
            
            for (const [source, metrics] of performance) {
                console.log(`\n📊 ${source.toUpperCase()}:`);
                console.log(`   Accuracy: ${(metrics.accuracy * 100).toFixed(1)}%`);
                console.log(`   Win Rate: ${(metrics.winRate * 100).toFixed(1)}%`);
                console.log(`   Avg Return: ${(metrics.avgReturn * 100).toFixed(2)}%`);
                console.log(`   Reliability: ${(metrics.reliability * 100).toFixed(1)}%`);
            }
            
            // Stop the system
            signalSystem.stop();
            
        }, 120000); // 2 minutes

        // Show active signals every 30 seconds
        setInterval(() => {
            const activeSignals = signalSystem.getActiveSignals();
            console.log(`\n📊 Active Signals: ${activeSignals.length}`);
            
            activeSignals.forEach(signal => {
                const age = (Date.now() - signal.timestamp) / 1000;
                console.log(`   ${signal.symbol} ${signal.direction} (${age.toFixed(0)}s ago, ${(signal.confidence * 100).toFixed(1)}%)`);
            });
            
        }, 30000);

    } catch (error) {
        console.error('❌ Error in Advanced Signals Demo:', error);
    }
}

// Mock WebSocket for testing (since we don't have real WebSocket in this environment)
if (typeof WebSocket === 'undefined') {
    console.log('🔧 Setting up mock WebSocket for testing...');
    
    global.WebSocket = class MockWebSocket {
        onopen?: () => void;
        onmessage?: (event: { data: string }) => void;
        onerror?: (error: any) => void;
        onclose?: () => void;
        
        constructor(url: string) {
            console.log(`📡 Mock WebSocket connecting to: ${url}`);
            
            // Simulate connection
            setTimeout(() => {
                if (this.onopen) this.onopen();
                
                // Send mock data every 5 seconds
                this.startMockData();
            }, 1000);
        }
        
        private startMockData(): void {
            const symbols = ['BTCUSDT', 'ETHUSDT', 'ADAUSDT', 'SOLUSDT'];
            
            setInterval(() => {
                const symbol = symbols[Math.floor(Math.random() * symbols.length)];
                const price = 50000 + Math.random() * 10000;
                const volume = 100 + Math.random() * 1000;
                
                const mockData = {
                    e: '24hrTicker',
                    E: Date.now(),
                    s: symbol,
                    c: price.toString(),
                    v: volume.toString(),
                    b: (price - 1).toString(),
                    a: (price + 1).toString(),
                    P: ((Math.random() - 0.5) * 10).toString(),
                    h: (price + Math.random() * 100).toString(),
                    l: (price - Math.random() * 100).toString()
                };
                
                if (this.onmessage) {
                    this.onmessage({ data: JSON.stringify(mockData) });
                }
            }, 3000 + Math.random() * 2000); // Every 3-5 seconds
        }
        
        close(): void {
            console.log('📡 Mock WebSocket closed');
            if (this.onclose) this.onclose();
        }
    } as any;
}

// Run the demo
if (require.main === module) {
    runAdvancedSignalsDemo().catch(console.error);
}
