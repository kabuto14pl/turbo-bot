...
class EnterpriseMLDashboard {
    ...
    constructor(port: number = 3001) {
        ...
        this.logger = new Logger('EnterpriseMLDashboard');
        this.port = port;
    ...
    private async sendMLStatus(socket: any): Promise<void> {
        try {
            const status = await this.getMLStatus();
            const metrics = await this.getMLMetrics();
            const performance = await this.getStrategyPerformance();
    ...
    private async getMLStatus(): Promise<any> {
    ...
    private async getMLMetrics(): Promise<any> {
