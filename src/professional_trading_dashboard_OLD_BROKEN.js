...
class ProfessionalTradingDashboard {
    ...
    async fetchCryptoPrices() {
        try {
            const binanceResponse = await axios_1.default.get('https://api.binance.com/api/v3/ticker/24hr', { timeout: 5000 });
            const symbols = ['BTCUSDT', 'ETHUSDT', 'SOLUSDT'];
            const out = [];
            for (const symbol of symbols) {
                const t = binanceResponse.data.find((x) => x.symbol === symbol);
                if (t) {
                    out.push({
                        symbol,
        ...
        try {
            const cg = await axios_1.default.get('https://api.coingecko.com/api/v3/simple/price', {
                params: {
    ...
    getMockCryptoData() {
        return [
            { symbol: 'BTCUSDT', price: 65000 + Math.random() * 10000, change: (Math.random() - 0.5) * 10, volume: 1000000 + Math.random() * 500000, high: 75000, low: 60000, source: 'mock' },
            { symbol: 'ETHUSDT', price: 3500 + Math.random() * 1000, change: (Math.random() - 0.5) * 8, volume: 500000 + Math.random() * 250000, high: 4200, low: 3200, source: 'mock' },
            { symbol: 'SOLUSDT', price: 150 + Math.random() * 100, change: (Math.random() - 0.5) * 12, volume: 100000 + Math.random() * 50000, high: 220, low: 130, source: 'mock' }
        ];
    }
...
chart - resize - handle;
{
    ...
    width: 12;
    px;
    background: linear - gradient(90, deg, transparent);
    var ;
...
hover;
{
    background: linear - gradient(90, deg, rgba(0, 212, 170, 0.2));
    var ;
    (--accent - primary), rgba(0, 212, 170, 0.2);
    ;
    opacity: 1;
    width: 16;
    px;
...
chart - resize - corner;
{
    ...
    z - index;
    101;
    background: linear - gradient(135, deg, transparent, 40 % );
    var ;
    (--accent - primary);
    50 % , transparent;
    60 % ;
...
hover;
{
    background: linear - gradient(135, deg, rgba(0, 212, 170, 0.2), 30 % );
    var ;
    (--accent - primary);
    50 % , rgba(0, 212, 170, 0.2);
    70 % ;
    ;
    opacity: 1;
    width: 30;
...
chart - resize - bottom;
{
    ...
    height: 12;
    px;
    background: linear - gradient(180, deg, transparent);
    var ;
...
hover;
{
    background: linear - gradient(180, deg, rgba(0, 150, 199, 0.2));
    var ;
    (--accent - secondary), rgba(0, 150, 199, 0.2);
    ;
    opacity: 1;
    height: 16;
    px;
...
strategy - card.active;
{
    ...
    var ;
    (--color - bull);
    background: linear - gradient(135, deg, rgba(0, 212, 170, 0.1), rgba(0, 150, 199, 0.05));
    box - shadow;
    0;
    6;
    px;
...
Handlowy < /span>
    < span;
style = "font-size: 12px; opacity: 0.7;" > v4;
.0;
.4 < /span>
    < /div>
    < div;
...
"pair-name" > BTCUSDT < /div>
    < div;
class {
}
"pair-price price-positive";
id = "btc-price" > $45, 247.82 < /div>
    < /div>
    < div;
class {
}
...
"pair-name" > ETHUSDT < /div>
    < div;
class {
}
"pair-price price-negative";
id = "eth-price" > $2, 789.45 < /div>
    < /div>
    < div;
class {
}
...
"symbol-name" > BTCUSDT < /div>
    < div;
class {
}
"current-price price-positive" > $45, 247.82 < /div>
    < div;
class {
}
"price-change price-positive" > +2.34 % /div>
    < /div>
    < div;
class {
}
...
"chart-resize-handle";
title = "Resize poziomy" > /div>
    < div;
id = "chart-resize-bottom";
class {
}
"chart-resize-bottom";
title = "Resize pionowy" > /div>
    < div;
id = "chart-resize-corner";
class {
}
"chart-resize-corner";
title = "Resize narożny" > /div>
    < /div>;
...
function initializeTerminal() {
    ...
    try {
        ...
        if (!Chart.registry || !Chart.registry.getScale('linear')) {
            try {
                Chart.register(Chart.CategoryScale, Chart.LinearScale, Chart.TimeScale, Chart.PointElement, Chart.LineElement, Chart.BarElement, Chart.Title, Chart.Tooltip, Chart.Legend, Chart.Filler);
