...
## P#217: Final Comprehensive Backtest — Parity with P#216 Live Bot

### P#217a: Config parity fixes (config.py)
- `TP_ATR_MULT` 2.5→4.0 (P#216 R:R fix)
- `PARTIAL_TP_ATR_L1` 1.5→2.0, `PARTIAL_TP_ATR_L2` 2.0→3.0 (P#216 partial TP)
- `FEE_GATE_MULTIPLIER` 1.5→2.0 (P#216 fee gate)
- `CONFIDENCE_FLOOR` 0.30→0.35 (P#216 confidence)
- `TP_CLAMP_MIN` 1.5→2.5, `TP_CLAMP_MAX` 4.5→6.0 (match TP=4.0 ATR)

### P#217b: Real Wilder ATR SuperTrend (data_downloader.py)
- Replaced simple SuperTrend (mult=2, no Wilder smoothing) with real implementation
- Wilder ATR (EWM alpha=1/period), multiplier=3, band continuity logic
- Current CSV data already correct (generated by fetch_historical.py with proper SuperTrend)

### P#217c: Momentum warmup fix (momentum_htf.py)
- `MTF_MIN_CANDLES` 200→50 (P#216 parity — 200 wasted 23% candles on warmup)

### P#217d: Strategy attribution in trade tables (runner.py)
- Added Strategy + Regime columns to per-pair winning/losing trade tables
- Added PER-STRATEGY BREAKDOWN section showing trades/PnL per strategy

### Backtest Results (1h, all 5 pairs):
- Portfolio: +$253.91 (+2.54%) on $10,000 | MaxDD: 1.2%
- SOL GridV2: 12 trades, PF 1.653, WR 66.7%, +$47.02
- Funding Arb: +$206.89 across all 5 pairs
- All 5 pairs net positive ✅
