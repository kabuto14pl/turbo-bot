...
const advanced_strategy_orchestrator_1 = require("./src/enterprise/orchestration/advanced_strategy_orchestrator");
const strategy_adapters_1 = require("./src/enterprise/orchestration/strategy_adapters");
...
class PhaseC2Validator {
    ...
    async test1_InitializeOrchestrator() {
        ...
        const config = {
            ...advanced_strategy_orchestrator_1.DefaultOrchestrationConfig,
            strategies,
        ...
        this.orchestrator = new advanced_strategy_orchestrator_1.AdvancedStrategyOrchestrator(config);
    ...
    async test2_RegisterMultipleStrategies() {
    ...
    async test3_IntegrateRealTimeData() {
        ...
        this.marketDataEngine = new real_time_market_data_engine_1.RealTimeMarketDataEngine(dataConfig);
        // Connect market data to orchestrator
        this.marketDataEngine.on('marketData', async (data) => {
            if (this.orchestrator) {
    ...
    async test4_TestSignalAggregation() {
    ...
    async test5_TestMemoryOptimization() {
    ...
    async test6_TestDynamicStrategySwitching() {
    ...
    async test7_TestPerformanceMonitoring() {
    ...
    async test8_ValidateSuccessCriteria() {
    ...
    createTestStrategies() {
        ...
        strategyNames.forEach(name => {
            const config = strategy_adapters_1.CommonStrategyConfigs[name];
            if (config) {
                const strategy = strategy_adapters_1.StrategyFactory.createMockStrategy(name);
    ...
    setupEventListeners() {
        if (!this.orchestrator)
            return;
        this.orchestrator.on('signalGenerated', (signal) => {
        ...
        this.orchestrator.on('memoryWarning', (stats) => {
