...
exports.EnterpriseEnsembleStrategyEngine = void 0;
const events_1 = require("events");
const enterprise_ml_performance_monitor_1 = require("./enterprise_ml_performance_monitor");
class EnterpriseEnsembleStrategyEngine extends events_1.EventEmitter {
    constructor() {
        super();
        this.isActive = false;
        // Component tracking
        this.registeredStrategies = new Map();
        this.registeredMLModels = new Map();
        this.componentPerformance = new Map();
        // Decision tracking
        this.decisionHistory = [];
        this.timeframeAnalysis = new Map();
        // Dynamic weights
        this.dynamicWeights = {
            strategies: new Map(),
            mlModels: new Map(),
        ...
        this.mlMonitor = enterprise_ml_performance_monitor_1.EnterpriseMLPerformanceMonitor.getInstance();
    ...
    registerMLModel(modelName, modelConfig) {
    ...
    async generateEnsembleDecision(strategySignals, mlPredictions, marketData) {
    ...
    async analyzeByTimeframe(strategySignals, mlPredictions) {
    ...
    calculateEnsembleWeights(strategySignals, mlPredictions) {
        ...
        if (this.config.dynamicWeighting.enabled) {
            ...
            // Apply bounds
            strategiesWeight = Math.max(this.config.dynamicWeighting.minWeight, Math.min(this.config.dynamicWeighting.maxWeight, strategiesWeight));
            mlWeight = Math.max(this.config.dynamicWeighting.minWeight, Math.min(this.config.dynamicWeighting.maxWeight, mlWeight));
            // Normalize
    ...
    performEnsembleVoting(strategySignals, mlPredictions, weights) {
        let buyScore = 0;
        let sellScore = 0;
        let holdScore = 0;
        let totalWeight = 0;
    ...
    async applyRiskAdjustments(votingResult, marketData) {
    ...
    calculateConsensus(strategySignals, mlPredictions) {
    ...
    calculateUncertainty(strategySignals, mlPredictions) {
    ...
    createSafeDefaultDecision(strategySignals, mlPredictions) {
        return {
            action: 'HOLD',
            confidence: 0.1,
            strength: 0.1,
    ...
    calculateComponentWeight(performance) {
        ...
        // Apply bounds
        return Math.max(this.config.dynamicWeighting.minWeight, Math.min(this.config.dynamicWeighting.maxWeight, totalScore));
    }
    normalizeWeights() {
    ...
    calculateTimeframeTrend(strategies, mlPredictions) {
        let bullishScore = 0;
        let bearishScore = 0;
        strategies.forEach(signal => {
            if (signal.signal === 'BUY')
                bullishScore += signal.confidence * signal.strength;
            else if (signal.signal === 'SELL')
                bearishScore += signal.confidence * signal.strength;
        });
        mlPredictions.forEach(prediction => {
            if (prediction.direction === 'UP')
                bullishScore += prediction.confidence * prediction.probability;
            else if (prediction.direction === 'DOWN')
                bearishScore += prediction.confidence * prediction.probability;
        });
    ...
    calculateTimeframeStrength(strategies, mlPredictions) {
        const allStrengths = [
    ...
    calculateTimeframeReliability(timeframe) {
    ...
    updateComponentTracking(decision) {
...
exports.EnterpriseEnsembleStrategyEngine = EnterpriseEnsembleStrategyEngine;
exports.default = EnterpriseEnsembleStrategyEngine;
